...
本轮实施和审核记录见[接口收紧方案](../iteration/design-dispatcher-interface-tightening.md)。当前默认集合已包含六个 basic_flight；其他技能的能力状态仍须按各自实现证据判断。

## 基础飞行接入结果

后续维护先读[执行总纲](../iteration/design-dispatcher-migration-execution-order.md)；此前实现完成过 S0–S6；当前已按用户要求将控制职责收回 dispatcher，修正版未重跑
...
执行证据见 [RPC 方案](../iteration/design-dispatcher-l4-rpc-integration.md)与
[飞行方案 §11](../iteration/design-dispatcher-basic-flight-migration.md#11-完成记录2026-09-17)。
