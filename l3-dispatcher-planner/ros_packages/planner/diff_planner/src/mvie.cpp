...
double MVIE::costMVIE(void *data, const double *x, double *g, const int n) {
        ...
        Eigen::Map<const Eigen::Vector3d> p(x_map.data());
        Eigen::Map<const Eigen::Vector3d> rtd(x_map.data() + 3);
        Eigen::Map<const Eigen::Vector3d> cde(x_map.data() + 6);
    ...
    bool MVIE::maxVolInsEllipsoid(const Eigen::MatrixX4d &hPoly, Ellipsoid &ellipsoid) {
        ...
        clp.setZero();
        clp(3) = -1.0;
        const double maxdepth = -math_utils::sdlp::linprog<4>(clp, Alp, blp, xlp);
        ...
        *pPenaltyWt = 1.0e+3;
        ...
        int ret = lbfgs::lbfgs_optimize(static_cast<int>(x.size()), x.data(), &minCost, &costMVIE,
                                        nullptr, nullptr, optData, &paramsMVIE);
