...
namespace diff_planner
{
      ...
      double &final_cost)
  {
          ...
          result == lbfgs::LBFGS_STOP)
      {
        ...
        if (!flag_swarm_too_close)
        {
          ...
          else if (finelyCheckAndSetConstraintPoints(segments_nouse, jerkOpt_, false) == CHK_RET::OBS_FREE)
          {
            ...
            ROS_INFO("free start_pt: (%.2f, %.2f, %.2f) collided: %d", jerkOpt_.getTraj().getPos(0.0).x(), jerkOpt_.getTraj().getPos(0.0).y(), jerkOpt_.getTraj().getPos(0.0).z(),
                    (int)(grid_map_->getOccupancy(jerkOpt_.getTraj().getPos(0.0))));
            // double max_sfc_violation = 0.0;
            // if (use_corridor_unknown_avoid_ && wei_sfc_corridor_ > 0.0  && !checkSfcCorridorFeasibility(jerkOpt_, max_sfc_violation))
            // {
            //   flag_still_unsafe = true;
            //   restart_nums++;
            //   PRINTF_COND("\033[32miter=%d,time(ms)=%5.3f, SFC corridor violated (max=%.3f > tol=%.3f), keep optimizing\n\033[0m",
            //               iter_num_, time_ms, max_sfc_violation, sfc_corridor_violation_tol_);
            // }
            // else
            // {
            //   flag_success = true;
          ...
          else
          {
            ...
            ROS_INFO("collied start_pt: (%.2f, %.2f, %.2f) collided: %d", jerkOpt_.getTraj().getPos(0.0).x(), jerkOpt_.getTraj().getPos(0.0).y(), jerkOpt_.getTraj().getPos(0.0).z(),
                    (int)(grid_map_->getOccupancy(jerkOpt_.getTraj().getPos(0.0))));
          }
        }
        else
    ...
    } while ((flag_still_unsafe && restart_nums < 8) ||
             (flag_force_return && force_stop_type_ == STOP_FOR_REBOUND && rebound_times <= 20));

    return flag_success;
  }

  bool PolyTrajOptimizer::checkSfcCorridorFeasibility(const poly_traj::MinJerkOpt &pt_data, double &max_violation)
  {
    max_violation = 0.0;
    if (wei_sfc_corridor_ <= 0.0 || sfc_init_data_.hPolytopes.empty())
      return true;

    const int N = static_cast<int>(pt_data.get_T1().size());
    if (sfc_init_data_.hPolyIdx.size() < N)
      return true;

    const int K = cps_num_prePiece_;
    Eigen::Matrix<double, 6, 1> beta0;
    double s1, s2, s3, s4, s5, step;

    for (int i = 0; i < N; ++i)
    {
      const int poly_idx = sfc_init_data_.hPolyIdx(i);
      if (poly_idx < 0 || poly_idx >= static_cast<int>(sfc_init_data_.hPolytopes.size()))
        continue;

      const auto &hPoly = sfc_init_data_.hPolytopes[poly_idx];
      const Eigen::Matrix<double, 6, 3> &c = pt_data.get_b().block<6, 3>(i * 6, 0);
      step = pt_data.get_T1()(i) / K;
      s1 = 0.0;
      ...
      for (int j = 0; j <= K; ++j)
      {
        ...
        {
          const Eigen::Vector3d outerNormal = hPoly.block<1, 3>(k, 0).transpose();
          const double violaPos = outerNormal.dot(pos) + hPoly(k, 3);
          if (violaPos > max_violation)
            max_violation = violaPos;
        }
    ...
    return max_violation <= sfc_corridor_violation_tol_;
  }
      ...
      int id_cps_end, PtsChk_t &pts_check)
  {
    ...
    const double RES = grid_map_->getResolution(), RES_2 = RES / 2;
    Eigen::VectorXd durations = traj.getDurations();
    if (!durations.allFinite() || durations.minCoeff() <= 0.0) {
      ROS_ERROR("computePointsToCheck: invalid trajectory durations (NaN/inf/non-positive), abort.");
      for (int i = 0; i < durations.size(); ++i)
        ROS_ERROR("  durations[%d]=%.6f", i, durations(i));
      return false;
    }
    ...
    while (true)
    {
      if (t > DURATION)
      {
        if (catch_goal_ && pts_check.size() > 0)
        {
        ...
        {
          ROS_ERROR("Failed to get points list to check (0x01). catch_goal_=%d, pts_check.size()=%d", catch_goal_, (int)pts_check.size());
          pts_check.clear();
          return false;
        }
      }
      ...
      if (!std::isfinite(next_t_stp) || !std::isfinite(t_step)) {
        ROS_ERROR("computePointsToCheck: non-finite t_step=%.6f next_t_stp=%.6f, abort.", t_step, next_t_stp);
        return false;
      }
      if (t >= next_t_stp)
      {
      ...
      const bool flag_first_init /*= true*/)
  {
    ...
    int i_end = ConstraintPoints::two_thirds_id(init_points, catch_goal_); // only check closed 2/3 points.
  ...
  {
    /* Gradient off: skip the EGO-like rebound detection entirely. Mid-optimization
     * transient collisions are resolved by the SFC corridor constraint; the final
     * validation is done by finelyCheckAndSetConstraintPoints after convergence. */
    if (!use_obs_grad_)
      return false;
    ...
    int i_end = ConstraintPoints::two_thirds_id(cps_.points, catch_goal_); // only check closed 2/3 points.
  ...
  void PolyTrajOptimizer::addPVAJGradCost2CT(EIGENVEC &gdT, Eigen::VectorXd &costs, const int &K)
  {
    ...
    for (int i = 0; i < N; ++i)
    {
      ...
      for (int j = 0; j <= K; ++j)
      {
        ...
        // collision (EGO-like rebound gradient; off = corridor-only avoidance)
        if (use_obs_grad_ && obstacleGradCostP(i_dp, pos, gradp, costp))
        {
        ...
        // SFC corridor (H-polytope position constraint)
        if (sfcCorridorGradCostP(i, pos, gradp, costp))
        {
          gradViolaPc = beta0 * gradp.transpose();
          gradViolaPt = alpha * gradp.transpose() * vel;
          jerkOpt_.get_gdC().block<6, 3>(i * 6, 0) += omg * step * gradViolaPc;
        ...
        // SFC waypoint attractor (junction overlap interior point)
        if (sfcWaypointAttractorGradCostP(i, j, K, N, pos, gradp, costp))
        {
          gradViolaPc = beta0 * gradp.transpose();
          gradViolaPt = alpha * gradp.transpose() * vel;
          jerkOpt_.get_gdC().block<6, 3>(i * 6, 0) += omg * step * gradViolaPc;
          gdT(i) += omg * (costp / K + step * gradViolaPt);
          costs(5) += omg * step * costp;
        }
  ...
  bool PolyTrajOptimizer::sfcCorridorGradCostP(const int piece_i,
                                               const Eigen::Vector3d &p,
                                               Eigen::Vector3d &gradp,
                                               double &costp)
  {
    if (!use_corridor_unknown_avoid_) {
      return false;
    }

    gradp.setZero();
    costp = 0.0;
    if (wei_sfc_corridor_ <= 0.0 || piece_i >= sfc_init_data_.hPolyIdx.size())
      return false;

    const int poly_idx = sfc_init_data_.hPolyIdx(piece_i);
    if (poly_idx < 0 || poly_idx >= static_cast<int>(sfc_init_data_.hPolytopes.size()))
      return false;
    ...
    {
      const Eigen::Vector3d outerNormal = hPoly.block<1, 3>(k, 0).transpose();
      const double violaPos = outerNormal.dot(p) + hPoly(k, 3);
      if (violaPos > 0.0)
      {
        const double violaPos2 = violaPos * violaPos;
        gradp += wei_sfc_corridor_ * 3.0 * violaPos2 * outerNormal;
        costp += wei_sfc_corridor_ * violaPos2 * violaPos;
        ret = true;
      }
    }
    return ret;
  ...
  bool PolyTrajOptimizer::sfcWaypointAttractorGradCostP(const int piece_i,
                                                      const int j,
                                                      const int K,
                                                      const int N,
                                                      const Eigen::Vector3d &p,
                                                      ...
                                                      double &costp)
  {
    ...
    gradp.setZero();
    costp = 0.0;
    if (wei_sfc_wp_attractor_ <= 0.0 || !sfc_wp_attractor_ready_)
      return false;

    const bool is_waypoint = (j == 0) && (piece_i != 0);
    const bool is_end = (j == K) && (piece_i != N - 1);
    if (!is_waypoint && !is_end)
      return false;

    const int idx = is_end ? piece_i : piece_i - 1;
    if (idx < 0 || idx >= waypoint_attractor_.cols())
      return false;

    const Eigen::Vector3d p_a = p - waypoint_attractor_.col(idx);
    const double violaAtt =
        p_a.squaredNorm() - waypoint_attractor_dead_d_(idx) * waypoint_attractor_dead_d_(idx);
    if (violaAtt <= 0.0)
      return false;

    const double violaAtt2 = violaAtt * violaAtt;
    gradp = wei_sfc_wp_attractor_ * 6.0 * violaAtt2 * p_a;
    costp = wei_sfc_wp_attractor_ * violaAtt2 * violaAtt;
    return true;
  }
  ...
  bool PolyTrajOptimizer::obstacleGradCostP(const int i_dp,
                                            ...
                                            double &costp)
  {
    if (i_dp == 0 || i_dp > ConstraintPoints::two_thirds_id(cps_.points, catch_goal_)) // only apply to first 2/3
      return false;
  ...
  {
    nh.param("manager/use_corridor_unknown_avoid", use_corridor_unknown_avoid_, false);
    nh.param("optimization/constraint_points_perPiece", cps_num_prePiece_, -1);
    nh.param("optimization/use_obstacle_gradient", use_obs_grad_, false);
    ...
    nh.param("optimization/weight_sfc_corridor", wei_sfc_corridor_, 0.0);
    nh.param("optimization/weight_sfc_wp_attractor", wei_sfc_wp_attractor_, 0.0);
    nh.param("optimization/sfc_corridor_violation_tol", sfc_corridor_violation_tol_, 0.2);
  }
  ...
  void PolyTrajOptimizer::setIfTouchGoal(const bool catch_goal) { catch_goal_ = catch_goal; }
  ...
  bool PolyTrajOptimizer::buildSfcWaypointAttractors()
  {
    sfc_wp_attractor_ready_ = false;
    waypoint_attractor_.resize(3, 0);
    waypoint_attractor_dead_d_.resize(0);
    h_overlap_polytopes_.clear();
    sfc_init_data_.hOverlapPolytopes.clear();
    sfc_init_data_.waypoint_attractor.resize(3, 0);
    sfc_init_data_.waypoint_attractor_dead_d.resize(0);

    const int poly_num = static_cast<int>(sfc_init_data_.hPolytopes.size());
    if (poly_num < 2)
    {
      sfc_wp_attractor_ready_ = true;
      return true;
    }

    const int size_corridor = poly_num - 1;
    waypoint_attractor_.resize(3, size_corridor);
    waypoint_attractor_dead_d_.resize(size_corridor);
    h_overlap_polytopes_.resize(size_corridor);
    sfc_init_data_.hOverlapPolytopes.resize(size_corridor);
    sfc_init_data_.waypoint_attractor.resize(3, size_corridor);
    sfc_init_data_.waypoint_attractor_dead_d.resize(size_corridor);

    for (int i = 0; i < size_corridor; ++i)
    {
      Eigen::MatrixX4d cur_ih(sfc_init_data_.hPolytopes[i].rows() + sfc_init_data_.hPolytopes[i + 1].rows(), 4);
      cur_ih.topRows(sfc_init_data_.hPolytopes[i].rows()) = sfc_init_data_.hPolytopes[i];
      cur_ih.bottomRows(sfc_init_data_.hPolytopes[i + 1].rows()) = sfc_init_data_.hPolytopes[i + 1];
      h_overlap_polytopes_[i] = cur_ih;
      sfc_init_data_.hOverlapPolytopes[i] = cur_ih;

      Eigen::Vector3d interior;
      const double depth = corridor_gen::findInteriorDist(cur_ih, interior);
      const double dead_d = depth / 2.0;
      if (dead_d <= corridor_gen::kEpsilon || std::isinf(dead_d) || std::isnan(dead_d))
      {
        ROS_WARN("[PolyTrajOptimizer] buildSfcWaypointAttractors failed at junction %d, depth=%.4f", i, depth);
        return false;
      }

      waypoint_attractor_.col(i) = interior;
      waypoint_attractor_dead_d_(i) = dead_d;
      sfc_init_data_.waypoint_attractor.col(i) = interior;
      sfc_init_data_.waypoint_attractor_dead_d(i) = dead_d;
    }

    sfc_wp_attractor_ready_ = true;
    ROS_INFO("[PolyTrajOptimizer] Built %d SFC waypoint attractors", size_corridor);
    return true;
  }

  void PolyTrajOptimizer::setInitSfcCorridor(const SfcCorridorInitData &data)
  {
    sfc_init_data_ = data;
    if (!buildSfcWaypointAttractors())
      ROS_WARN("[PolyTrajOptimizer] SFC corridor set but waypoint attractor build failed");
  }
