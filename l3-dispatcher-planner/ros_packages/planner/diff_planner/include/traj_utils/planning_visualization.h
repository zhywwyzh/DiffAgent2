...
namespace diff_planner
{
  ...
  private:
    ...
    ros::Publisher sfc_corridor_pub;
  ...
  public:
    ...
    void displayGuidePathList(const vector<Eigen::Vector3d> &guide_pts, const double scale, int id = 0);
    ...
    void displayOptPolyJunctionList(const vector<Eigen::Vector3d> &junc_pts, bool success, int id);
    void displayOptResult(const Eigen::MatrixXd &cstr_pts, const vector<Eigen::Vector3d> &junc_pts,
                          bool success, int id);
    void displayAStarList(std::vector<std::vector<Eigen::Vector3d>> a_star_paths, int id);
    void displaySfcCorridor(const std::vector<Eigen::Matrix<double, Eigen::Dynamic, 4>> &hPolys,
