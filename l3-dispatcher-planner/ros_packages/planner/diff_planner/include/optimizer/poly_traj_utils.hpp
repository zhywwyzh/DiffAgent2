...
namespace poly_traj
{
    ...
    public:
        ...
        inline bool getPartialTrajectoryByTime(const double start_tt, const double end_tt,
                                               Trajectory &out_traj) const
        {
            const double total_dur = getTotalDuration();
            if (start_tt < 0.0 || end_tt <= start_tt || end_tt > total_dur + 1e-6)
                return false;
            ...
            {
                double t0 = end_tt;
                const int piece_idx = locatePieceIdx(t0);
                if (piece_idx == 0)
                {
                    Piece new_pie = pieces[piece_idx];
            ...
            const int piece_idx = locatePieceIdx(t0);
            const int piece_end_idx = locatePieceIdx(local_end_t);

            const double t02 = t0 * t0;
            const double t03 = t02 * t0;
            const double t04 = t03 * t0;
            const double t05 = t04 * t0;
            Eigen::MatrixXd coef_mat = pieces[piece_idx].getCoeffMat();
            ...
            cvt_m << 1, 0, 0, 0, 0, 0,
                ...
                t05, t04, t03, t02, t0, 1;
            coef_mat = coef_mat * cvt_m.transpose();
            const double p1_t = std::min(pieces[piece_idx].getDuration() - t0, end_tt - start_tt);
            out_traj.emplace_back(p1_t, coef_mat);
            if (piece_idx == piece_end_idx)
                return true;

            for (int i = piece_idx + 1; i < piece_end_idx; ++i)
                out_traj.emplace_back(pieces[i]);

            out_traj.emplace_back(local_end_t, pieces[piece_end_idx].getCoeffMat());
            return true;
        }
