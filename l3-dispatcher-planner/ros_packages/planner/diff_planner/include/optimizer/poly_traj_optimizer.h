...
namespace diff_planner
{
  ...
  struct SfcCorridorInitData
  {
    std::vector<Eigen::Matrix<double, Eigen::Dynamic, 4>> hPolytopes;
    std::vector<Eigen::Matrix<double, Eigen::Dynamic, 4>> hOverlapPolytopes;
    ...
    {
      hPolytopes.clear();
      hOverlapPolytopes.clear();
      waypoint_attractor.resize(3, 0);
      waypoint_attractor_dead_d.resize(0);
      hPolyIdx.resize(0);
      sfc_piece_num = 0;
    }
  };
  ...
  public:
    ...
    static inline int two_thirds_id(Eigen::MatrixXd &points, const bool catch_goal)
    {
      return catch_goal ? points.cols() - 1 : points.cols() - 1 - (points.cols() - 2) / 3;
    }
  ...
  private:
    ...
    bool use_corridor_unknown_avoid_; // whether to consider unknown space as obstacles in corridor construction and optimization
    SfcCorridorInitData sfc_init_data_;
    Eigen::Matrix3Xd waypoint_attractor_;
    Eigen::VectorXd waypoint_attractor_dead_d_;
    std::vector<Eigen::Matrix<double, Eigen::Dynamic, 4>> h_overlap_polytopes_;
    bool sfc_wp_attractor_ready_{false};
    ...
    bool use_obs_grad_{false};                                    // EGO-like A*-guided rebound obstacle gradient (default off: corridor-only)
    ...
    double wei_sfc_corridor_, wei_sfc_wp_attractor_, sfc_corridor_violation_tol_;                  // SFC corridor / wp attractor
  ...
  public:
    ...
    /* Risk-adaptive velocity bound (SUPER risk mechanism): overwritten per replan. */
    void setMaxVelBound(const double v) { max_vel_ = v; }
    void setIfTouchGoal(const bool catch_goal);
    void setConstraintPoints(ConstraintPoints cps);
    void setUseMultitopologyTrajs(bool use_multitopology_trajs);
    void setInitSfcCorridor(const SfcCorridorInitData &data);

    /* helper functions */
    inline const SfcCorridorInitData &getInitSfcCorridor(void) const { return sfc_init_data_; }
    inline const Eigen::Matrix3Xd &getWaypointAttractor(void) const { return waypoint_attractor_; }
    inline const Eigen::VectorXd &getWaypointAttractorDeadD(void) const { return waypoint_attractor_dead_d_; }
    inline const std::vector<Eigen::Matrix<double, Eigen::Dynamic, 4>> &getHOverlapPolytopes(void) const
    {
      return h_overlap_polytopes_;
    }
    inline bool isSfcWaypointAttractorReady(void) const { return sfc_wp_attractor_ready_; }
    ...
    bool checkSfcCorridorFeasibility(const poly_traj::MinJerkOpt &pt_data, double &max_violation);
  ...
  private:
    ...
    bool buildSfcWaypointAttractors();
    ...
    bool sfcCorridorGradCostP(const int piece_i,
                              const Eigen::Vector3d &p,
                              Eigen::Vector3d &gradp,
                              double &costp);

    bool sfcWaypointAttractorGradCostP(const int piece_i,
                                       const int j,
                                       const int K,
                                       const int N,
                                       const Eigen::Vector3d &p,
