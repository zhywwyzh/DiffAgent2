...
namespace plan_manage {
  ...
  void EGOPlannerManager::initPlanModules(ros::NodeHandle &nh)
  {
    ...
    poly_traj_opt_.reset(new ego_planner::traj_opt::PolyTrajOptimizer);
      ...
      const std::vector<Eigen::Vector3d> &via_points)
  {
    ...
    ego_planner::traj_opt::poly_traj::MinJerkOpt initMJO;
    ...
    if (ret == ego_planner::traj_opt::PolyTrajOptimizer::CHK_RET::ERR || ret == ego_planner::traj_opt::PolyTrajOptimizer::CHK_RET::TIME_LIM)
    {
      continous_failures_count_++;
      failure_cnt_++;
      ROS_WARN_STREAM("[EGOPlanner] ego_rebound_replan_pipeline_result drone_id=" << pp_.drone_id
    ...
    last_search_path_.clear();
    for (const auto& astar_seg : AstarPathes)
      for (const auto& pt : astar_seg)
        last_search_path_.push_back(pt);
    ...
    ego_planner::traj_opt::poly_traj::MinJerkOpt best_MJO;
    ...
    if (pp_.use_multitopology_trajs)
    {
      ...
      int selected_id = 0;
      ego_planner::traj_opt::poly_traj::Trajectory initTraj = initMJO.getTraj();
      ...
      const std::vector<Eigen::Vector3d> &via_points)
  {
    ...
    else /*** case 2: path initialization ***/
    {
      ...
      if (!via_points.empty())
      {
        /* Waypoint-window chain: thread the repaired window points through the
         * initial path in travel order; only the window-final goal is the hard
         * terminal (touch_goal). Segments between via points may cross
         * obstacles - the constraint-point A* repair and the obstacle gradient
         * (rebound) own that later. */
        for (const auto& wp : via_points)
          best_path.push_back(wp);
      }
      else if ((pathes && !pathes->empty()) && !flag_random_init)
      {
  ...
  bool EGOPlannerManager::setLocalTrajFromOpt(const ego_planner::traj_opt::poly_traj::MinJerkOpt &opt, const bool touch_goal, const bool set_uk_info /* = false */)
  {
    ego_planner::traj_opt::poly_traj::Trajectory traj = opt.getTraj();
  ...
  bool EGOPlannerManager::OnePieceTrajGen(Eigen::Vector3d start_pos, Eigen::Vector3d end_pos)
  {
    ...
    ego_planner::traj_opt::poly_traj::MinJerkOpt OnePieceMJO;
  ...
  ego_planner::traj_opt::poly_traj::Trajectory EGOPlannerManager::OnePieceTrajGen(
  ...
  bool EGOPlannerManager::EmergencyStop(Eigen::Vector3d stop_pos)
  {
    ...
    tailState = headState;
    ego_planner::traj_opt::poly_traj::MinJerkOpt stopMJO;
