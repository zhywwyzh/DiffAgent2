...
namespace traj_opt {
  /* main planning API */
  bool ego_planner::traj_opt::PolyTrajOptimizer::optimizeTrajectory(
      ...
      double &final_cost)
  {
    ...
    ego_planner::traj_opt::lbfgs::lbfgs_parameter_t lbfgs_params;
    ego_planner::traj_opt::lbfgs::lbfgs_load_default_parameters(&lbfgs_params);
    ...
    do
    {
      ...
      /* ---------- optimize ---------- */
      t1 = ros::Time::now();
      int result = ego_planner::traj_opt::lbfgs::lbfgs_optimize(
          variable_num_,
          x_init,
          &final_cost,
          ego_planner::traj_opt::PolyTrajOptimizer::costFunctionCallback,
          ego_planner::traj_opt::PolyTrajOptimizer::stepSizeBound,
          ego_planner::traj_opt::PolyTrajOptimizer::earlyExitCallback,
          this,
          &lbfgs_params);
      ...
      if (normalityCheck())
      {
        if (result == ego_planner::traj_opt::lbfgs::LBFGS_CONVERGENCE ||
            result == ego_planner::traj_opt::lbfgs::LBFGSERR_MAXIMUMITERATION ||
            result == ego_planner::traj_opt::lbfgs::LBFGS_ALREADY_MINIMIZED ||
            result == ego_planner::traj_opt::lbfgs::LBFGS_STOP ||
            result == ego_planner::traj_opt::lbfgs::LBFGSERR_MAXIMUMLINESEARCH)
        {
          if (result != ego_planner::traj_opt::lbfgs::LBFGS_CONVERGENCE &&
              result != ego_planner::traj_opt::lbfgs::LBFGS_ALREADY_MINIMIZED &&
              result != ego_planner::traj_opt::lbfgs::LBFGS_STOP)
          {
            /* NOTE! */
            // ego_planner::traj_opt::lbfgs::LBFGSERR_MAXIMUMLINESEARCH is always caused by small return conditions:
            ROS_WARN_COND(VERBOSE_OUTPUT, "Accepted error. Return = %d, %s", result, ego_planner::traj_opt::lbfgs::lbfgs_strerror(result));
          }
          ...
          {
            ego_planner::traj_opt::PolyTrajOptimizer::CHK_RET ret =
        ...
        else if (result == ego_planner::traj_opt::lbfgs::LBFGSERR_CANCELED)
        {
        ...
        else
        {
          ...
          ROS_WARN("[EGOOptimizer] ego_optimizer_solver_error lbfgs_result=%d lbfgs_error=%s piece_num=%d variable_num=%d total_iter_num=%d restart_nums=%d rebound_times=%d",
                   result, ego_planner::traj_opt::lbfgs::lbfgs_strerror(result), piece_num_, variable_num_,
                   total_iter_num_, restart_nums, rebound_times);
          ROS_WARN_COND(VERBOSE_OUTPUT, "Solver error. Return = %d, %s. Skip this planning.", result, ego_planner::traj_opt::lbfgs::lbfgs_strerror(result));
          state.act = OptFsm::FAILED_RET;
        }
      }
      else
  ...
  bool ego_planner::traj_opt::PolyTrajOptimizer::optimizeTrajectoryShapeOnly(
      const Eigen::MatrixXd &iniState, const Eigen::MatrixXd &finState,
      const Eigen::MatrixXd &initInnerPts, const Eigen::VectorXd &initT,
      const int CPsNumPerPiece, double &final_cost)
  {
    ...
    do
    {
      ...
      int result = ego_planner::traj_opt::lbfgs::lbfgs_optimize(
          ...
          x_init,
          &final_cost,
          ego_planner::traj_opt::PolyTrajOptimizer::ShapeOnlyCostFunctionCallback,
          ego_planner::traj_opt::PolyTrajOptimizer::stepSizeBound,
          ego_planner::traj_opt::PolyTrajOptimizer::earlyExitCallback,
          this,
          &lbfgs_params);
  ...
  double ego_planner::traj_opt::PolyTrajOptimizer::costFunctionCallback(void *func_data, const double *x, double *grad, const int n)
  {
    ego_planner::traj_opt::PolyTrajOptimizer *opt = reinterpret_cast<ego_planner::traj_opt::PolyTrajOptimizer *>(func_data);
  ...
  /* callbacks by the L-BFGS optimizer */
  double ego_planner::traj_opt::PolyTrajOptimizer::ShapeOnlyCostFunctionCallback(void *func_data, const double *x, double *grad, const int n)
  {
    ego_planner::traj_opt::PolyTrajOptimizer *opt = reinterpret_cast<ego_planner::traj_opt::PolyTrajOptimizer *>(func_data);
  ...
  int ego_planner::traj_opt::PolyTrajOptimizer::earlyExitCallback(void *func_data, const double *x, const double *g, const double fx, const double xnorm, const double gnorm, const double step, int n, int k, int ls)
  {
    ego_planner::traj_opt::PolyTrajOptimizer *opt = reinterpret_cast<ego_planner::traj_opt::PolyTrajOptimizer *>(func_data);
  ...
  double ego_planner::traj_opt::PolyTrajOptimizer::stepSizeBound(void *func_data, const double *xp, const double *d, const int n)
  {
    ego_planner::traj_opt::PolyTrajOptimizer *opt = reinterpret_cast<ego_planner::traj_opt::PolyTrajOptimizer *>(func_data);
  ...
  bool ego_planner::traj_opt::PolyTrajOptimizer::computePointsToCheck(
      const ego_planner::traj_opt::poly_traj::Trajectory &traj,
  ...
  bool ego_planner::traj_opt::PolyTrajOptimizer::normalityCheck()
  {
    // Shape check
  ...
  ego_planner::traj_opt::PolyTrajOptimizer::CHK_RET ego_planner::traj_opt::PolyTrajOptimizer::finelyCheckAndSetConstraintPoints(
      ...
      const bool flag_first_init /*= true*/)
  {
    ...
    ego_planner::traj_opt::poly_traj::Trajectory traj = pt_data.getTraj();
    ...
    for (size_t seg_id = 0; seg_id < segment_ids.size(); ++seg_id)
    {
      ...
      ego_planner::path_searching::dyn_a_star::ASTAR_RET ret = a_star_->AstarSearch(reso, in, out);
      if (ret == ego_planner::path_searching::dyn_a_star::ASTAR_RET::SUCCESS)
      {
        a_star_pathes.push_back(a_star_->getPath());
      }
      else if (ret == ego_planner::path_searching::dyn_a_star::ASTAR_RET::NO_PATH && seg_id + 1 < segment_ids.size()) // connect the next segment
      {
        segment_ids[seg_id].second = segment_ids[seg_id + 1].second;
        segment_ids.erase(segment_ids.begin() + seg_id + 1);
        --seg_id;
      }
      else if (ret == ego_planner::path_searching::dyn_a_star::ASTAR_RET::TIME_LIM)
      {
        // No need more trials
  ...
  bool ego_planner::traj_opt::PolyTrajOptimizer::roughlyCheckConstraintPoints(void)
  {
    ...
    if (flag_new_obs_valid)
    {
      ...
      for (size_t seg_id = 0; seg_id < segment_ids.size(); ++seg_id)
      {
        ...
        ego_planner::path_searching::dyn_a_star::ASTAR_RET ret = a_star_->AstarSearch(/*(in-out).norm()/10+0.05*/ map_->cur_->getResolution(), in, out);
        if (ret == ego_planner::path_searching::dyn_a_star::ASTAR_RET::SUCCESS)
        {
          a_star_pathes.push_back(a_star_->getPath());
        }
  ...
  Eigen::Vector3d ego_planner::traj_opt::PolyTrajOptimizer::tryExtendAndChkP(Eigen::Vector3d p, Eigen::Vector3d v, Eigen::Vector3d q)
  {
  ...
  bool ego_planner::traj_opt::PolyTrajOptimizer::allowRebound(void) // zxzxzx
  {
    // criterion 1
    if (iter_num_ < 3)
      return false;
  ...
  void ego_planner::traj_opt::PolyTrajOptimizer::prepareFittedCurve()
  {
  ...
  void ego_planner::traj_opt::PolyTrajOptimizer::computeVelLim(const Eigen::MatrixXd &iniState, Eigen::MatrixXd finState)
  {
  ...
  /* multi-topo support */
  std::vector<ConstraintPoints> ego_planner::traj_opt::PolyTrajOptimizer::distinctiveTrajs(vector<std::pair<int, int>> segments)
  {
  ...
  void ego_planner::traj_opt::PolyTrajOptimizer::RealT2VirtualT(const Eigen::VectorXd &RT, EIGENVEC &VT)
  {
  ...
  template <typename EIGENVEC>
  void ego_planner::traj_opt::PolyTrajOptimizer::VirtualT2RealT(const EIGENVEC &VT, Eigen::VectorXd &RT)
  {
  ...
  void ego_planner::traj_opt::PolyTrajOptimizer::VirtualTGradCost(
  ...
  template <typename EIGENVEC>
  void ego_planner::traj_opt::PolyTrajOptimizer::initAndGetSmoothnessGradCost2PT(EIGENVEC &gdT, double &cost)
  {
    jerkOpt_.initGradCost(gdT, cost);
  }

  void ego_planner::traj_opt::PolyTrajOptimizer::initAndGetSmoothnessGradCost2P(double &cost)
  {
    jerkOpt_.initGradCostPOnly(cost);
  }

  template <typename EIGENVEC>
  void ego_planner::traj_opt::PolyTrajOptimizer::addPVAJGradCost2CT(EIGENVEC &gdT, Eigen::VectorXd &cost_rec, const int &K)
  {
    //
    int N = gdT.size();
  ...
  void ego_planner::traj_opt::PolyTrajOptimizer::addPGradCost2C(Eigen::VectorXd &costs, const int &N, const int &K)
  {
    int i_dp = 0;
    double costp;
  ...
  bool ego_planner::traj_opt::PolyTrajOptimizer::obstacleGradCostP(const int i_dp,
  ...
  bool ego_planner::traj_opt::PolyTrajOptimizer::ESDFGradCostP(const int i_dp,
                                        const Eigen::Vector3d &p,
                                        Eigen::Vector3d &gradp,
                                        double &costp)
  {
  ...
  bool ego_planner::traj_opt::PolyTrajOptimizer::restrictplaneGradCostP(const int i_piece,
                                                 const Eigen::Vector3d &p,
                                                 Eigen::Vector3d &gradp,
                                                 double &costp)
  {
  ...
  bool ego_planner::traj_opt::PolyTrajOptimizer::FixUnknwonPosGradCostP(const int cps_id,
                                                 const Eigen::Vector3d &p,
  ...
  bool ego_planner::traj_opt::PolyTrajOptimizer::CurveFittingGradCostP(const int cps_id,
                                                const Eigen::Vector3d &p,
                                                Eigen::Vector3d &gradp,
                                                double &costp)
  {
  ...
  bool ego_planner::traj_opt::PolyTrajOptimizer::swarmGradCostP(const int i_dp,
                                         const double t,
                                         const Eigen::Vector3d &p,
                                         const Eigen::Vector3d &v,
                                         Eigen::Vector3d &gradp,
  ...
  bool ego_planner::traj_opt::PolyTrajOptimizer::feasibilityGradCostV(const int i_dp,
                                               const Eigen::Vector3d &v,
                                               Eigen::Vector3d &gradv,
                                               double &costv)
  {
  ...
  bool ego_planner::traj_opt::PolyTrajOptimizer::feasibilityGradCostA(const int i_dp,
  ...
  bool ego_planner::traj_opt::PolyTrajOptimizer::feasibilityGradCostJ(const int i_dp,
  ...
  bool ego_planner::traj_opt::PolyTrajOptimizer::feasibilityGradCostS(const int i_dp,
  ...
  void ego_planner::traj_opt::PolyTrajOptimizer::distanceSqrVarianceWithGradCost2p(const Eigen::MatrixXd &ps,
                                                            Eigen::MatrixXd &gdp,
                                                            double &var)
  {
    int N = ps.cols() - 1;
  ...
  void ego_planner::traj_opt::PolyTrajOptimizer::lengthVarianceWithGradCost2p(const Eigen::MatrixXd &ps,
                                                       const int n,
                                                       Eigen::MatrixXd &gdp,
                                                       double &var)
  {
  ...
  /* helper functions */
  void ego_planner::traj_opt::PolyTrajOptimizer::setParam(ros::NodeHandle &nh)
  {
  ...
  void ego_planner::traj_opt::PolyTrajOptimizer::setEnvironment(const ego_planner::plan_env::MapManager::Ptr map)
  {
    map_ = map;

    a_star_.reset(new ego_planner::path_searching::dyn_a_star::AStar);
    a_star_->initAstar(map_, Eigen::Vector3i(100, 100, 100));
  }

  void ego_planner::traj_opt::PolyTrajOptimizer::setControlPoints(const Eigen::MatrixXd &points)
  {
    cps_.points = points;
  }

  void ego_planner::traj_opt::PolyTrajOptimizer::setSwarmTrajs(SwarmTrajData *swarm_trajs_ptr) { swarm_trajs_ = swarm_trajs_ptr; }

  void ego_planner::traj_opt::PolyTrajOptimizer::setDroneId(const int drone_id) { drone_id_ = drone_id; }

  void ego_planner::traj_opt::PolyTrajOptimizer::setIfTouchGoal(const bool touch_goal) { touch_goal_ = touch_goal; }

  void ego_planner::traj_opt::PolyTrajOptimizer::setConstraintPoints(ConstraintPoints cps) { cps_ = cps; }

  void ego_planner::traj_opt::PolyTrajOptimizer::setUseMultitopologyTrajs(bool use_multitopology_trajs) { multitopology_data_.use_multitopology_trajs = use_multitopology_trajs; }

  void ego_planner::traj_opt::PolyTrajOptimizer::setMaxVelAcc(double max_vel, double max_acc) { max_vel_ = max_vel, max_acc_ = max_acc; }

  void ego_planner::traj_opt::PolyTrajOptimizer::setCPsNumPerPiece(const int N) { cps_num_prePiece_ = N, cps_num_prePiece_Long_ = N; }

  void ego_planner::traj_opt::PolyTrajOptimizer::setPlanParametersCopy(const PlanParameters &pp_cpy) { pp_cpy_ = pp_cpy; }
