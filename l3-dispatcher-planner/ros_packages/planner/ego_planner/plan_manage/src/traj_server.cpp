...
void ego_planner::plan_manage::TrajServer::initTrajServer(ros::NodeHandle& node) {
  node_ = node;
  ...
  last_yaw_ = 0.0;
  last_yawdot_ = 0.0;
  percep_utils_ = std::make_shared<vis_utils::PerceptionUtils>(node_);
  std::thread cmd_thread(ego_planner::plan_manage::TrajServer::cmdThread, this);
  cmd_thread.detach();
}

void ego_planner::plan_manage::TrajServer::feedDog() {
  heartbeat_time_ = ros::Time::now();
  do_once_ = true;
}

void ego_planner::plan_manage::TrajServer::resetYawLookforward(Eigen::Vector3d pos, bool enable_pre_yaw) {
  std::cout << "[ego_planner::plan_manage::TrajServer] resetYawLookforward" << std::endl;
  ...
  pre_yaw_pending_ = enable_pre_yaw;
}

void ego_planner::plan_manage::TrajServer::setYaw(double des_yaw, double cur_yaw, Eigen::Vector3d pos,
                                                  bool look_forward, uint8_t control_mode, uint8_t path_mode) {
  panorama_yaw_active_ = false;
  ...
  if (path_mode == quadrotor_msgs::LocalGoalSet::YAW_PATH_KEEP_DIRECTION) {
    ...
    while (unwrapped_cur_yaw - last_yaw_ > M_PI)
      unwrapped_cur_yaw -= 2 * M_PI;
    while (unwrapped_cur_yaw - last_yaw_ < -M_PI)
      unwrapped_cur_yaw += 2 * M_PI;
...
void ego_planner::plan_manage::TrajServer::setPanoramaYaw(double des_yaw, double cur_yaw,
                                                          const Eigen::Vector3d& hold_pos) {
...
void ego_planner::plan_manage::TrajServer::syncYawFromOdom(const double yaw, const std::string& source) {
  double normalized_yaw = yaw;
  while (normalized_yaw > M_PI)
    normalized_yaw -= 2 * M_PI;
  while (normalized_yaw < -M_PI)
    normalized_yaw += 2 * M_PI;
  ...
  time_rec_.has_init = false;
  if (source.empty()) {
    ROS_INFO_STREAM_THROTTLE(0.5, "[ego_planner::plan_manage::TrajServer] sync yaw from odom: " << normalized_yaw);
  } else {
    ROS_INFO_STREAM_THROTTLE(
        0.5, "[ego_planner::plan_manage::TrajServer] sync yaw from odom: " << normalized_yaw << " source=" << source);
  }
}

void ego_planner::plan_manage::TrajServer::setTrajectory(const ego_planner::traj_opt::poly_traj::Trajectory& traj,
                                                         double start_time) {
  panorama_yaw_active_ = false;
  traj_ = traj;
  ...
  pre_yaw_completed_event_ = false;
  time_rec_.has_init = false;
}

void ego_planner::plan_manage::TrajServer::reset() {
  receive_traj_ = false;
  pre_yaw_pending_ = false;
  pre_yaw_completed_event_ = false;
  traj_state_ = TrajState::IDLE;
  time_rec_.has_init = false;
}

bool ego_planner::plan_manage::TrajServer::consumePreYawCompletedEvent() {
  if (!pre_yaw_completed_event_)
    return false;
  pre_yaw_completed_event_ = false;
  return true;
}

std::pair<double, double> ego_planner::plan_manage::TrajServer::calculate_yaw(double t_cur, const Eigen::Vector3d& pos,
                                                                               double dt) {
  ...
  if (!yaw_given_.look_forward &&
      yaw_given_.control_mode != quadrotor_msgs::LocalGoalSet::YAW_MODE_PANORAMA) {
    double target_yaw = yaw_given_.yaw;
    while (target_yaw > M_PI)
      target_yaw -= 2 * M_PI;
    while (target_yaw < -M_PI)
      target_yaw += 2 * M_PI;
    const double observed_error = atan2(sin(target_yaw - observed_yaw_), cos(target_yaw - observed_yaw_));
    yaw_given_.reach_given_yaw_ = fabs(observed_error) < 0.01;
  ...
  {
    Eigen::Vector3d dir = t_cur + time_forward_ <= traj_duration_ ? traj_.getPos(t_cur + time_forward_) - pos
    ...
    const bool panorama_stopped = yaw_given_.control_mode != quadrotor_msgs::LocalGoalSet::YAW_MODE_PANORAMA ||
...
void ego_planner::plan_manage::TrajServer::publish_cmd(Vector3d p, Vector3d v, Vector3d a, Vector3d j, double y,
                                                       double yd, uint8_t yaw_control_mode) {
  ...
  cmd.yaw = y;
  cmd.yaw_dot = yd;
  cmd.yaw_control_mode = yaw_control_mode;
  pos_cmd_pub_.publish(cmd);

  last_pos_ = p;
}

void ego_planner::plan_manage::TrajServer::resetLastPos(const Eigen::Vector3d pos) { last_pos_ = pos; }

void ego_planner::plan_manage::TrajServer::cmdThread(void* obj) {
  ego_planner::plan_manage::TrajServer* tsvr = reinterpret_cast<ego_planner::plan_manage::TrajServer*>(obj);
...
void ego_planner::plan_manage::TrajServer::cmdFun() {
  ...
  if (!receive_traj_) {
    if (!yaw_given_.reach_given_yaw_) {
      ...
      if (!time_rec_.has_init) {
        ...
        // publish
        const uint8_t yaw_control_mode =
            yaw_given_.control_mode == quadrotor_msgs::LocalGoalSet::YAW_MODE_PANORAMA
                ? quadrotor_msgs::PositionCommand::YAW_CONTROL_TRACK
                : quadrotor_msgs::PositionCommand::YAW_CONTROL_TARGET;
        publish_cmd(pos, vel, acc, jer, yaw_yawdot.first, yaw_yawdot.second, yaw_control_mode);
    ...
    if (traj_state_ == TrajState::IDLE) {
      traj_init_pos_ = traj_.getPos(0.0);
      const double sample_time = std::min(time_forward_, traj_duration_);
      Eigen::Vector3d initial_dir = traj_.getPos(sample_time) - traj_init_pos_;
      if (initial_dir.head<2>().norm() < 1e-3)
        initial_dir = traj_.getVel(0.0);
      if (pre_yaw_pending_ && yaw_given_.look_forward && initial_dir.head<2>().norm() > 1e-3) {
        traj_init_yaw_ = atan2(initial_dir(1), initial_dir(0));
        traj_state_ = TrajState::PRE_YAW;
        time_rec_.has_init = false;
        ROS_INFO("[traj_server] pre_yaw_start target=%.3f current=%.3f", traj_init_yaw_, observed_yaw_);
      } else {
        pre_yaw_pending_ = false;
    ...
    if (traj_state_ == TrajState::PRE_YAW) {
      publish_cmd(traj_init_pos_, vel, acc, jer, traj_init_yaw_, 0.0,
                  quadrotor_msgs::PositionCommand::YAW_CONTROL_TARGET);
      percep_utils_->setPose(traj_init_pos_, traj_init_yaw_);
      vector<Eigen::Vector3d> l1, l2;
      percep_utils_->getFOV(l1, l2);
      drawFOV(l1, l2, cmd_vis_pub_);

      double yaw_error = traj_init_yaw_ - observed_yaw_;
      while (yaw_error > M_PI)
        yaw_error -= 2.0 * M_PI;
      while (yaw_error < -M_PI)
        yaw_error += 2.0 * M_PI;
      if (fabs(yaw_error) <= 5.0 * M_PI / 180.0) {
        start_time_ = time_now.toSec();
        traj_state_ = TrajState::EXECUTING_TRAJ;
        pre_yaw_pending_ = false;
        pre_yaw_completed_event_ = true;
        last_yaw_ = observed_yaw_;
        last_yawdot_ = 0.0;
        time_rec_.has_init = false;
        ROS_INFO("[traj_server] pre_yaw_complete yaw=%.3f", observed_yaw_);
      }
      return;
    }
...
void ego_planner::plan_manage::TrajServer::drawFOV(const std::vector<Eigen::Vector3d>& list1,
                                                   const std::vector<Eigen::Vector3d>& list2, ros::Publisher& pub,
  ...
  const size_t corner_count = std::min<size_t>(4, std::min(list1.size(), list2.size()));
  Eigen::Vector3d origin_center = Eigen::Vector3d::Zero();
  Eigen::Vector3d far_center = Eigen::Vector3d::Zero();
  for (size_t i = 0; i < corner_count; ++i) {
    origin_center += list1[i];
    far_center += list2[i];
  }
  origin_center /= static_cast<double>(corner_count);
  far_center /= static_cast<double>(corner_count);

  visualization_msgs::Marker forward;
  forward.header = mk.header;
  forward.ns = "planned_forward";
  forward.id = 1;
  forward.type = visualization_msgs::Marker::ARROW;
  forward.action = visualization_msgs::Marker::ADD;
  forward.pose.orientation.w = 1.0;
  forward.scale.x = 0.06;
  forward.scale.y = 0.14;
  forward.scale.z = 0.18;
  forward.color.r = 1.0;
  forward.color.g = 0.0;
  forward.color.b = 1.0;
  forward.color.a = 1.0;
  geometry_msgs::Point origin_point;
  origin_point.x = origin_center.x();
  origin_point.y = origin_center.y();
  origin_point.z = origin_center.z();
  geometry_msgs::Point far_point;
  far_point.x = far_center.x();
  far_point.y = far_center.y();
  far_point.z = far_center.z();
  forward.points = {origin_point, far_point};
  pub.publish(forward);
}
