...
void GridMap::initMap(ros::NodeHandle &nh)
{
  ...
  node_.param("grid_map/cloud_odom_sync_tolerance", mp_.cloud_odom_sync_tolerance_, 0.15);
  node_.param("grid_map/cloud_outlier_radius", mp_.cloud_outlier_radius_, 0.18);
  node_.param("grid_map/cloud_outlier_min_neighbors", mp_.cloud_outlier_min_neighbors_, 2);
  node_.param("grid_map/cloud_self_filter_radius", mp_.cloud_self_filter_radius_, 0.10);
  node_.param("grid_map/cloud_self_filter_z_min", mp_.cloud_self_filter_z_min_, -0.30);
  node_.param("grid_map/cloud_self_filter_z_max", mp_.cloud_self_filter_z_max_, 0.40);
  ...
  visPtr_ = std::make_shared<ego_planner::plan_env::visualization::Visualization>(node_);
...
void GridMap::updateOccupancyPC(void *obj, const sensor_msgs::PointCloud2ConstPtr &img)
{
  ...
  const size_t raw_count = latest_cloud.points.size();
  map->last_cloud_raw_count_.store(raw_count);

  if (latest_cloud.points.size() == 0)
    return;

  if (map->mp_.cloud_outlier_radius_ > 0.0 && map->mp_.cloud_outlier_min_neighbors_ > 0)
  {
    pcl::PointCloud<pcl::PointXYZ>::Ptr filtered_cloud(new pcl::PointCloud<pcl::PointXYZ>);
    pcl::RadiusOutlierRemoval<pcl::PointXYZ> filter;
    filter.setInputCloud(latest_cloud.makeShared());
    filter.setRadiusSearch(map->mp_.cloud_outlier_radius_);
    filter.setMinNeighborsInRadius(map->mp_.cloud_outlier_min_neighbors_);
    filter.filter(*filtered_cloud);
    latest_cloud.swap(*filtered_cloud);
  }
  map->last_cloud_radius_count_.store(latest_cloud.points.size());
  ...
  ROS_INFO_THROTTLE(1.0,
                    "[EGO_CLOUD_FILTER] map=%s raw=%zu radius_kept=%zu radius_removed=%zu self_removed=%llu raycast=%d skew=%.3f",
                    map->mp_.name_.c_str(), raw_count, latest_cloud.points.size(), raw_count - latest_cloud.points.size(),
                    static_cast<unsigned long long>(map->last_cloud_self_filtered_count_.load()), map->md_.proj_points_cnt_,
                    map->last_cloud_odom_skew_.load());
  t3 = ros::Time::now();
...
void GridMap::odomCallback(const nav_msgs::OdometryConstPtr &odom)
{
  ...
  md_.camera_pos_(2) = cam_T(2, 3);
  md_.camera_r_m_ = cam_T.block<3, 3>(0, 0);
  md_.body_pos_(0) = odom->pose.pose.position.x;
  md_.body_pos_(1) = odom->pose.pose.position.y;
  md_.body_pos_(2) = odom->pose.pose.position.z;
  md_.odom_stamp_ = odom->header.stamp;
...
{
  mtx_.lock();
  const bool has_odom = md_.has_odom_;
  const ros::Time odom_stamp = md_.odom_stamp_;
  mtx_.unlock();
  if (!has_odom)
  {
  ...
  if (!img->header.stamp.isZero() && !odom_stamp.isZero())
  {
    const double skew = std::abs((img->header.stamp - odom_stamp).toSec());
    last_cloud_odom_skew_.store(skew);
    if (skew > mp_.cloud_odom_sync_tolerance_)
    {
      ROS_WARN_THROTTLE(1.0, "[%s] drop cloud: stamp/odom skew=%.3fs > %.3fs",
                        mp_.name_.c_str(), skew, mp_.cloud_odom_sync_tolerance_);
      return;
    }
  }

  // Serialize updates: detached workers can concurrently corrupt the ring buffer.
  updateOccupancyPC(this, img);
}
...
void GridMap::projectPC(pcl::PointCloud<pcl::PointXYZ> &pc)
{
  ...
  md_.proj_points_cnt_ = 0;
  uint64_t self_filtered_count = 0;
  int skip_pix = mp_.skip_pixel_;
  ...
  for (int i = 0; i < pc.points.size(); i += 1)
  {
    ...
    const Eigen::Vector3d body_delta = proj_pt - md_.body_pos_;
    if (body_delta.head<2>().norm() < mp_.cloud_self_filter_radius_ &&
        body_delta.z() >= mp_.cloud_self_filter_z_min_ &&
        body_delta.z() <= mp_.cloud_self_filter_z_max_)
    {
      ++self_filtered_count;
      continue;
    }
  ...
  last_cloud_self_filtered_count_.store(self_filtered_count);
  last_cloud_projected_count_.store(md_.proj_points_cnt_);
}
