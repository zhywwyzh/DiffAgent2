...
#define QUARD_MINIMIZER2_LBFGS(qm, u, du, v, dv) \
    ...
    inline int line_search_morethuente(const int n,
                                       double *x,
                                       double *f,
                                       double *g,
                                       const double *s,
        ...
        */
        ...
        for (;;)
        {
            ...
            */
            ...
            //     stepmax = std::max(stepmax, s[i] * *stp);
            // printf("stepmax=%f, snorm=%f, *stp=%f\n", stepmax, snorm, *stp);
            ...
            *f = cd->proc_evaluate(cd->instance, x, g, cd->n);
            ...
            if (cd->proc_stepbound && *stp == *stpmax) // zxzx
            {
                /* Allow the step anyway. */
                return count;
            }
            ...
            */
            ...
            while (loop == 1)
            {
                ...
                if (cd.proc_stepbound)
                {
                    ...
                    if (step >= step_max)
                        // step = step_max / 2.0; // zxzx
                        step = step_max / 1.01; // zxzx
                }
...
} // namespace ego_planner // namespace lbfgs
