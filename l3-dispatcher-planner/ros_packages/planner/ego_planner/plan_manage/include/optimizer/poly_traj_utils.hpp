...
namespace poly_traj {
    ...
    public:
        ...
        inline int locatePieceIdx(double &t) const
        {
            ...
            if (idx == N)
            {
                ...
                if (t > pieces[idx].getDuration() + 1e-5)
                {
                    printf("\033[31mERROR! t=%f>duration=%f! 0x01\033[0m\n", t, pieces[idx].getDuration());
                }
            }
            return idx;
        }
        ...
        inline std::pair<int, double> locatePieceIdxWithRatio(double &t) const
        {
            ...
            if (idx == N)
            {
                ...
                if (t > pieces[idx].getDuration() + 1e-5)
                {
                    printf("\033[31mERROR! t=%f>duration=%f! 0x03\033[0m\n", t, pieces[idx].getDuration());
                }
            }
    ...
    public:
        ...
        inline void create(const int &n, const int &p, const int &q)
        {
            ...
            lowerBw = p;
            upperBw = q;
            MatData = Eigen::VectorXd::Zero(N * (lowerBw + upperBw + 1));
            return;
        }
    ...
    public:
        ...
        {
            return MatData((i - j + upperBw) * N + j);
        }
    ...
    public:
        ...
        inline double getTrajJerkCost(const int PieceEnd)
        {
            ...
            part_cost.resize(PieceEnd);
            for (int i = 0; i < PieceEnd; i++)
            {
                part_cost(i) = 36.0 * b.row(6 * i + 3).squaredNorm() * T1(i) +
                ...
                objective += part_cost(i) * T3(i);
                // objective += (36.0 * b.row(6 * i + 3).squaredNorm() * T1(i) +
                //              144.0 * b.row(6 * i + 4).dot(b.row(6 * i + 3)) * T2(i) +
                //              192.0 * b.row(6 * i + 4).squaredNorm() * T3(i) +
                //              240.0 * b.row(6 * i + 5).dot(b.row(6 * i + 3)) * T3(i) +
                //              720.0 * b.row(6 * i + 5).dot(b.row(6 * i + 4)) * T4(i) +
                //              720.0 * b.row(6 * i + 5).squaredNorm() * T5(i)) * T3(i);
            }
            return objective;
        }
        ...
        inline double getTrajAccCost()
        {
            ...
            for (int i = 0; i < N; i++)
            {
                part_cost(i) = 4.0 * b.row(6 * i + 2).squaredNorm() * T1(i) +
                               12.0 * b.row(6 * i + 2).dot(b.row(6 * i + 3)) * T2(i) +
                               12.0 * b.row(6 * i + 3).squaredNorm() * T3(i) +
                               16.0 * b.row(6 * i + 2).dot(b.row(6 * i + 4)) * T3(i) +
                               36.0 * b.row(6 * i + 3).dot(b.row(6 * i + 4)) * T4(i) +
                               20.0 * b.row(6 * i + 2).dot(b.row(6 * i + 5)) * T4(i) +
                               28.8 * b.row(6 * i + 4).squaredNorm() * T5(i) +
                               48.0 * b.row(6 * i + 3).dot(b.row(6 * i + 5)) * T5(i) +
                               80.0 * b.row(6 * i + 4).dot(b.row(6 * i + 5)) * T6(i) +
                               400.0 / 7.0 * b.row(6 * i + 5).squaredNorm() * T7(i);
                objective += part_cost(i) * T2(i);
            }
            return objective;
        }

        template <typename EIGENVEC>
        inline void addGradJbyT(EIGENVEC &gdT, const int PieceEnd) const
        {
            for (int i = 0; i < PieceEnd; i++)
            {
                double part_grad = 36.0 * b.row(6 * i + 3).squaredNorm() +
                ...
                gdT(i) += part_grad * T3(i) + 3 * part_cost(i) * T2(i);
        ...
        template <typename EIGENMAT>
        inline void addGradJbyC(EIGENMAT &gdC, const int PieceEnd) const
        {
            for (int i = 0; i < PieceEnd; i++)
            {
        ...
        template <typename EIGENMAT>
        inline void addGradAbyC(EIGENMAT &gdC) const
        {
            for (int i = 0; i < N; i++)
            {
                gdC.row(6 * i + 5) += (20.0 * b.row(6 * i + 2) * T4(i) +
                                       48.0 * b.row(6 * i + 3) * T5(i) +
                                       80.0 * b.row(6 * i + 4) * T6(i) +
                                       800.0 / 7.0 * b.row(6 * i + 5) * T7(i)) *
                                      T2(i);
                gdC.row(6 * i + 4) += (16.0 * b.row(6 * i + 2) * T3(i) +
                                       36.0 * b.row(6 * i + 3) * T4(i) +
                                       57.6 * b.row(6 * i + 4) * T5(i) +
                                       80.0 * b.row(6 * i + 5) * T6(i)) *
                                      T2(i);
                gdC.row(6 * i + 3) += (12.0 * b.row(6 * i + 2) * T2(i) +
                                       24.0 * b.row(6 * i + 3) * T3(i) +
                                       36.0 * b.row(6 * i + 4) * T4(i) +
                                       48.0 * b.row(6 * i + 5) * T5(i)) *
                                      T2(i);
                gdC.row(6 * i + 2) += (8.0 * b.row(6 * i + 2) * T1(i) +
                                       12.0 * b.row(6 * i + 3) * T2(i) +
                                       16.0 * b.row(6 * i + 4) * T3(i) +
                                       20.0 * b.row(6 * i + 5) * T4(i)) *
                                      T2(i);
            }
            return;
        }
        ...
        inline void getGrad2TP(EIGENVEC &gdT,
                               EIGENMAT &gdInPs)
        {
            Eigen::MatrixXd b = gdC; // zxzx
            solveAdjGradC(b);
            addPropCtoT(b, gdT);
            addPropCtoP(b, gdInPs);
        }

        template <typename EIGENMAT>
        inline void getGrad2P(EIGENMAT &gdInPs)
        {
            Eigen::MatrixXd b = gdC; // zxzx
            solveAdjGradC(b);
            addPropCtoP(b, gdInPs);
...
// printf( "gdInPs=%d\n", gdInPs.size() );
#if ONLY_USE_2_3
            const int PieceEnd = N - floor(N / 3);
#else
            const int PieceEnd = N;
#endif

            gdT.setZero();
            gdC.setZero();
            cost = getTrajJerkCost(PieceEnd);
            addGradJbyT(gdT, PieceEnd);
            addGradJbyC(gdC, PieceEnd);
        }

        inline void initGradCostPOnly(double &cost)
        {
#if ONLY_USE_2_3
            const int PieceEnd = N - floor(N / 3);
#else
...
} // namespace ego_planner // namespace poly_traj
