...
struct MappingParameters
{
  ...
  double cloud_odom_sync_tolerance_{0.15};
  double cloud_outlier_radius_{0.18};
  int cloud_outlier_min_neighbors_{2};
  double cloud_self_filter_radius_{0.10};
  double cloud_self_filter_z_min_{-0.30};
  double cloud_self_filter_z_max_{0.40};
...
public:
  ...
  inline uint64_t getLastCloudRawCount() const { return last_cloud_raw_count_.load(); }
  inline uint64_t getLastCloudRadiusCount() const { return last_cloud_radius_count_.load(); }
  inline uint64_t getLastCloudSelfFilteredCount() const { return last_cloud_self_filtered_count_.load(); }
  inline uint64_t getLastCloudProjectedCount() const { return last_cloud_projected_count_.load(); }
  inline double getLastCloudOdomSkew() const { return last_cloud_odom_skew_.load(); }
  ...
  std::shared_ptr<ego_planner::plan_env::visualization::Visualization> visPtr_;
...
private:
  ...
  std::atomic<uint64_t> last_cloud_raw_count_{0};
  std::atomic<uint64_t> last_cloud_radius_count_{0};
  std::atomic<uint64_t> last_cloud_self_filtered_count_{0};
  std::atomic<uint64_t> last_cloud_projected_count_{0};
  std::atomic<double> last_cloud_odom_skew_{0.0};
