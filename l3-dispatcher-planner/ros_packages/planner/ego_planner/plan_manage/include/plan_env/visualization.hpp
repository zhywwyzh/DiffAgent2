...
class Visualization {
 ...
 public:
  ...
  void debug_a_float(const double& floatdata,
                    const TOPIC& topic) {
    auto got = publisher_map_.find(topic);
    if (got == publisher_map_.end()) {
      ros::Publisher pub = nh_.advertise<std_msgs::Float32>(topic, 100);
      publisher_map_[topic] = pub;
    }
        std_msgs::Float32 msg;
        msg.data = floatdata; // Example float value
  ...
  template <class TOPIC>
  void visualize_spheres(const std::vector<Eigen::Vector3d>& balls,
                       const TOPIC& topic,
                       const double radius,
                       const Color color = blue,
                       const double a = 0.2) {
  ...
  void visualize_texts(const std::vector<std::string> prefix_text, const TEXT& texts, 
                       const double scale, 
                       const TOPIC& topic, 
  ...
  template <class CUBES, class TOPIC>
  void visualize_cubes(const CUBES& cubes,
                       const TOPIC& topic,
                       const double& scale,
    ...
    marker.id = id;
    setMarkerColor(marker, color(3), color(0), color(1), color(2));
    setMarkerScale(marker, scale, scale, scale);
  ...
  void visualize_pairline(const PAIRLINE& pairline, const TOPIC& topic, 
                          const double scale = 0.02, const Color color = red) {
    if (pairline.empty()) return;
    auto got = publisher_map_.find(topic);
    if (got == publisher_map_.end()) {
      ros::Publisher pub =
          nh_.advertise<visualization_msgs::MarkerArray>(topic, 10);
    ...
    setMarkerScale(arrow_msg, scale, scale, scale);
    setMarkerColor(arrow_msg, color, 0.5);
  ...
  void visualize_arrows(const ARROWS& arrows, const TOPIC& topic, ego_planner::plan_env::visualization::Color color = yellow) {
    ...
    arrow_msg.points.resize(2);
    setMarkerPose(arrow_msg, 0, 0, 0);
    setMarkerScale(arrow_msg, 0.05, 0.12, 0);
    setMarkerColor(arrow_msg, color);
    visualization_msgs::MarkerArray arrow_list_msg;
...
} // namespace ego_planner  // namespace visualization
