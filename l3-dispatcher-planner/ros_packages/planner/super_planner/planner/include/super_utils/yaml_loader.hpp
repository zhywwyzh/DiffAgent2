...
public:
  explicit YamlLoader(const std::string& file_path) {
    path_ = file_path;
    SLOG_DEBUG("Load config file: {}", path_);
  }
  ...
  template <class T>
  bool LoadParam(const std::string& param_name, std::vector<T>& param_value, const std::vector<T> default_value = {},
                 const bool& required = false) {
    return loadParamInternal(param_name, param_value, default_value, required);
  }
...
private:
  ...
  bool loadParamInternal(const std::string& param_name, T& param_value, const T& default_value, bool required) {
    ...
    if (!tmp_node.IsNull()) {
      ...
      if constexpr (is_vector<T>::value) {
        if (tmp_node.Type() == YAML::NodeType::Sequence) {
          try {
            ...
            SLOG_DEBUG("Load param {} success:", param_name);
            printValue(param_value);
            return true;
          } catch (const YAML::BadConversion&) {
            SLOG_WARN("Type mismatch for param {}, using default value:", param_name);
      ...
      SLOG_WARN("Load param {} failed, use default value:", param_name);
      param_value = default_value;
      printValue(param_value);
      if (required) {
  ...
  template <typename T> void printValue(const T& param_value) {
    if constexpr (is_vector<T>::value) {
      ...
      for (const auto& elem : param_value) {
        value_str += fmt::format("{} ", elem);
      }
      value_str += "]";
      SLOG_DEBUG("{}", value_str);
    } else {
      SLOG_DEBUG("{}", param_value);
    }
  }
