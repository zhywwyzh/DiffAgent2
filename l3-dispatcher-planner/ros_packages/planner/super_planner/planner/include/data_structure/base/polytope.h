...
namespace geometry_utils {
    ...
    static bool SimplifySFC(const Vec3f& head_p, const Vec3f& tail_p,
                                  super_planner::geometry_utils::PolytopeVec& sfcs) {
        ...
        if (sfcs.size() > 2) {
            ...
            if (start_id < 0 || end_id < 0) {
                SLOG_WARN(" -- [EXPTrajOpt] Ill corridor! Forced return.");
                return false;
            }
            if (start_id >= end_id) {
                end_id = start_id;
            ...
            if (sfcs_new.size() > 2) {
                ...
                for (int i = 2; i < sfcs_new.size(); i++) {
                    ...
                    const double overlap_depth =
                            super_planner::geometry_utils::findInteriorDist(cross_poly.GetPlanes(), interior_pt);
