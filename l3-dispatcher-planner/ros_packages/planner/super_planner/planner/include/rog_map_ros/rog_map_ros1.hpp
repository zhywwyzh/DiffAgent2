...
namespace rog_map {
    ...
    class ROGMapROS :public ROGMap {
        ...
        void cloudCallback(const sensor_msgs::PointCloud2ConstPtr& cloud_msg) {
            ...
            if (cbk_t - robot_state_.rcv_time > cfg_.odom_timeout) {
                SLOG_WARN(" -- [ROS] Odom timeout, skip cloud callback.");
                return;
            }
            PointCloud tmp_pc;
        ...
        void updateCallback(const ros::TimerEvent& event) {
            if (map_empty_) {
                ...
                if (cfg_.ros_callback_en && (cur_t - last_print_t > 1.0)) {
                    SLOG_WARN(" -- [ROG WARN] No point cloud input, check the topic name.");
                    last_print_t = cur_t;
                }
                return;
            }
            ...
            if (rc_.unfinished_frame_cnt > 1) {
                SLOG_WARN(" -- [ROG WARN] Unfinished frame cnt > 1, the map may not work in real-time");
            }
        ...
        void vizCallback(const ros::TimerEvent& event) {
            ...
            if ((box_max - box_min).minCoeff() <= 0) {
                SLOG_WARN(" -- [ROGMap] Visualization range is too small.");
                return;
            }
    ...
    public:
        ...
        ROGMapROS(const ros::NodeHandle& nh, const std::string& cfg_path) :nh_(nh){
            cfg_ = super_planner::rog_map::Config(cfg_path);
            /* Launch-explicit cloud source: the super_planner.launch.xml
             * cloud_topic arg is injected as /super_planner_node/
             * ros_callback/cloud_topic; prefer it over the yaml value so
             * the launch is the single source of the map cloud. */
            ros::NodeHandle pnh("~");
            std::string cloud_topic_override;
            if (pnh.getParam("ros_callback/cloud_topic", cloud_topic_override)) {
                cfg_.cloud_topic = cloud_topic_override;
                SLOG_WARN("[ROG] cloud_topic overridden by launch -> {}", cfg_.cloud_topic);
            }
            init();
