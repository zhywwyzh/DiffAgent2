...
namespace traj_opt {
    ...
    public:
        ...
        super_planner::flatness::FlatnessMap quadrotot_flatness;
        ...
        // Limit conditions.
        double max_vel{0}, max_acc{0}, max_jerk{0}, max_omg{0}, max_acc_thr{0}, min_acc_thr{0}, max_vel_z{-1};
        /* Yaw-rate feasibility bound [rad/s]: the optimizer penalizes trajectory
         * points whose heading rate (d/dt atan2(vy,vx)) exceeds this, so the
         * online yaw follower can track the velocity heading. <=0 disables. */
        double max_yaw_rate{-1};
        // Penalty cost.
        double penna_scale{-1}, penna_vel{0}, penna_acc{0}, penna_jerk{0}, penna_omg{0}, penna_thr{0};
        // Weight of the yaw-rate feasibility penalty.
        double penna_yaw_rate{0};
        ...
        Config(const std::string & cfg_path, string ns) {
            ...
            loader.LoadParam("debug_log_en", print_optimizer_log, false);
            ...
            loader.LoadParam("debug_log_en", save_log_en, false);
            ...
            /* max_vel is injected by super_core::Config from
             * super_planner/risk/vel_max (single velocity-ceiling source). */
            loader.LoadParam("traj_opt/boundary/max_vel_z", max_vel_z, -1.0);
            ...
            loader.LoadParam("traj_opt/boundary/max_yaw_rate", max_yaw_rate, -1.0);
            ...
            loader.LoadParam("traj_opt" + ns + "penna_yaw_rate", penna_yaw_rate, 0.0);
            ...
            if (penna_scale > 0) {
                ...
                penna_yaw_rate = penna_yaw_rate * penna_scale;
