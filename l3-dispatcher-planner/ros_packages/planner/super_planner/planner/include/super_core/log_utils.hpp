...
namespace super_planner {
    ...
    public:
        ...
        void record(super_planner::ros_interface::RosInterface::Ptr &viz_ptr, const bool keyframe, const bool ambient) {
            viz_ptr->recordReplan(exp_traj, backup_traj, exp_yaw_traj, backup_yaw_traj, exp_sfc, backup_sfc,
                                  pc_for_sfc, keyframe, ambient);
        }
    ...
    public:
        ...
        void setGoal(const Vec3f &_goal_p, const double &_goal_yaw,
                     const super_planner::super_utils::RobotState &_robot) {
            robot_p = _robot.p;
        ...
        void replanExpTrajectory(const ExpTrajOpt::Ptr &exp_traj_opt, Trajectory &traj) {
            ...
            SLOG_DEBUG("Replan exp trajectory==========================");
            SLOG_DEBUG("init_state: {}", exp_init_state);
            SLOG_DEBUG("fina_state: {}", exp_fina_state);
            SLOG_DEBUG("sfcs: {}", exp_sfc.size());
            SLOG_DEBUG("init_ps: \n{}", exp_init_ps.size());
            SLOG_DEBUG("init_ts: {}", exp_init_t_vec.transpose());
        ...
        void replanBackupTrajectory(const BackupTrajOpt::Ptr &backup_traj_opt, Trajectory &traj) {
            if (exp_traj.empty() || backup_init_ps.size() == 0 || backup_init_t_vec.size() == 0
                ...
                SLOG_DEBUG("Backup traj is empty");
                return;
            }
            SLOG_DEBUG("Online backup trajectory==========================");
            SLOG_DEBUG("ts_min: {}", ts_min);
            SLOG_DEBUG("ts_max: {}", ts_max);
            SLOG_DEBUG("backup_init_ts: {}", backup_init_ts);
            SLOG_DEBUG("backup_init_ps: \n{}", backup_init_ps.size());
            SLOG_DEBUG("backup_init_t_vec: {}", backup_init_t_vec.size());
