...
namespace super_planner {
    ...
    public:
        ...
        bool cutPolyBySensingHorizon( const Vec3f & robot_p, const Vec3f & guide_p,
            ...
            SLOG_DEBUG("{}", new_plane.transpose());
            ...
            SLOG_DEBUG("123123124512======================");
            SLOG_DEBUG("{}", new_planes);
            poly.SetPlanes(new_planes);
            Vec3f it;
            //// h0*x + h1*y + h2*z + h3 <= 0
            if (!super_planner::geometry_utils::findInterior(new_planes,it)) {
                return false;
            }
            return true;
        }

        bool cutPolyByFov(const Vec3f & robot_p, const super_planner::super_utils::Quatf & robot_q,
            ...
            Vec3f interior;
            if (super_planner::geometry_utils::findInteriorDist(poly.GetPlanes(), interior) < small_x) {
                return false;
            }
        ...
        FOVChecker(const FOVType &fov_type,
            ...
            switch (cfg_.type) {
                case OMNI: {
                    ...
                    static double zup = sin(cfg_.upper_angle) * 5;
                    SLOG_DEBUG("zup: {}", zup);
                    static double rup = cos(cfg_.upper_angle) * 5;
                    SLOG_DEBUG("rup: {}", rup);
                    static double zdown = sin(cfg_.lower_angle) * 5;
                    SLOG_DEBUG("zdown: {}", zdown);
                    static double rdown = cos(cfg_.lower_angle) * 5;
                    SLOG_DEBUG("rdown: {}", rdown);
        ...
        void getFovCheckPlane(
            ...
            Eigen::Vector4d temp;
            super_planner::geometry_utils::FromPointsToPlane(four_pts.col(0), four_pts.col(1), t, temp);
            ...
            super_planner::geometry_utils::FromPointsToPlane(four_pts.col(2), four_pts.col(3), t, temp);
            if (temp.head(3).dot(fov_inner_pt) + temp(3) > 0) {
                temp = -temp;
            }
