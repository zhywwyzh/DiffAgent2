...
public:
  ...
  void
  initESDFMap(const Vec3i& half_prob_map_size_i, /* The input is half map size, to ensure the map size is always odds*/
              const double& prob_map_resolution, const double& temp_counter_map_resolution,
              const Vec3f& local_update_box, const bool& map_sliding_en, const double& sliding_thresh,
              const Vec3f& fix_map_origin, const double& unk_thresh);
  ...
  void getPositiveESDFPointCloud(const super_planner::rog_map::Vec3f& box_min_d,
                                 const super_planner::rog_map::Vec3f& box_max_d, const double& visualize_z,
                                 pcl::PointCloud<pcl::PointXYZI>& pcl_pc);

  void getNegativeESDFPointCloud(const super_planner::rog_map::Vec3f& box_min_d,
                                 const super_planner::rog_map::Vec3f& box_max_d, const double& visualize_z,
                                 pcl::PointCloud<pcl::PointXYZI>& pcl_pc);

private:
  template <typename F_get_val, typename F_set_val>
  void fillESDF(F_get_val f_get_val, F_set_val f_set_val, const int& start, const int& end, const int& dim,
                const int& id_l);
  ...
  void triggerJumpingEdge(const super_planner::rog_map::Vec3i& id_g, const super_planner::rog_map::GridType& from_type,
                          const super_planner::rog_map::GridType& to_type) override {}
