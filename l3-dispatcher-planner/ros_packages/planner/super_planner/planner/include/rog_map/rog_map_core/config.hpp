...
#define CMAKE_ROOT_DIR(name) (string(string(ROOT_DIR) + name))
    ...
    if (std::regex_search(input, cmakeRootDirRegex)) {
      ...
      SLOG_DEBUG("The pcd_name is remapped to {}", replaced);
    } else {
      replaced = input;
    }
    return replaced;
...
public:
  ...
  Config(const string& cfg_path, const string& name_space = "rog_map") {
    ...
    loader.LoadParam("debug_log_en", debug_log_en, false);
    ...
    loader.LoadParam(name_space + "/ros_callback/cloud_topic", cloud_topic, string("/cloud_registered_aug"));
    ...
    if (temp_vis_range.size() != 3) {
      ...
      if (visualization_range.minCoeff() <= 0) {
        SLOG_WARN(" -- [ROG] Visualization range is not set, visualization is disabled");
    ...
    /* Optional second inflation layer for the A* frontend: a precomputed
     * occupancy dilation at astar_inflation_step * inflation_resolution,
     * so the frontend hard constraint is an O(1) cell query instead of a
     * runtime neighbor dilation. 0 = disabled. */
    loader.LoadParam(name_space + "/astar_inflation_step", astar_inflation_step, 0);
    ...
    if (point_filt_num <= 0) {
      SLOG_WARN(" -- [ROG] point_filt_num should be larger or equal than 1, it is set to 1 now.");
      point_filt_num = 1;
    }
    ...
    if (batch_update_size <= 0) {
      SLOG_WARN(" -- [ROG] batch_update_size should be larger or equal than 1, it is set to 1 now.");
      batch_update_size = 1;
    }
    ...
    /* Keep the yaml-original values: resetMapSize remaps the float bounds
     * in place, and recomputeInflationDerived must restart from these
     * instead of compounding the remap on a copied config. */
    virtual_ground_height_yaml = virtual_ground_height;
    virtual_ceil_height_yaml = virtual_ceil_height;
...
#define logit(x) (log((x) / (1 - (x))))
    ...
    SLOG_DEBUG("\t[ROG] n_free: {}", n_free);
    SLOG_DEBUG("\t[ROG] n_occ: {}", n_occ);

    SLOG_DEBUG("\t[ROG] l_hit: {}", l_hit);
    SLOG_DEBUG("\t[ROG] l_miss: {}", l_miss);
    SLOG_DEBUG("\t[ROG] l_min: {}", l_min);
    SLOG_DEBUG("\t[ROG] l_max: {}", l_max);
    SLOG_DEBUG("\t[ROG] l_occ: {}", l_occ);
    SLOG_DEBUG("\t[ROG] l_free: {}", l_free);
      ...
      for (int dx = -unk_inflation_step; dx <= unk_inflation_step; dx++) {
        for (int dy = -unk_inflation_step; dy <= unk_inflation_step; dy++) {
          for (int dz = -unk_inflation_step; dz <= unk_inflation_step; dz++) {
            if (unk_inflation_step == 1 || dx * dx + dy * dy + dz * dz <= unk_inflation_step * unk_inflation_step) {
  ...
  int inflation_step{};
  /* Second inflation layer for the A* frontend (0 = disabled). */
  int astar_inflation_step{0};
  ...
  /* Yaml-original virtual bounds, captured before resetMapSize remaps them. */
  double virtual_ceil_height_yaml{}, virtual_ground_height_yaml{};
  int inf_virtual_ceil_height_id_g{}, inf_virtual_ground_height_id_g{};

  bool debug_log_en{false};
  ...
  void resetMapSize() {
    ...
    SLOG_DEBUG(" -- [RM-Config] inflation_ratio: {}", inflation_ratio);
#endif

    inflation_resolution = resolution * inflation_ratio;
    SLOG_DEBUG(" -- [RM-Config] inflation_resolution: {}", inflation_resolution);
    ...
    inf_half_map_size_i = (half_map_size_d / inflation_resolution).cast<int>() + (max_step + 1) * Vec3i::Ones();
    ...
    SLOG_DEBUG(" -- [ROG-Map] Init, resetMapSize: ORIGIN_AT_CENTER.");
#endif

#ifdef ORIGIN_AT_CORNER
    inf_virtual_ceil_height_id_g = static_cast<int>(floor(virtual_ceil_height / inflation_resolution)) - inflation_step;
    inf_virtual_ground_height_id_g =
        static_cast<int>(floor(virtual_ground_height / inflation_resolution)) + inflation_step;

    SLOG_DEBUG(" -- [ROG-Map] Init, resetMapSize: ORIGIN_AT_CORNER.");
#endif
    ...
    SLOG_DEBUG(" -- [ROG-Map] Init, resetMapSize: virtual_ceil_height: {}, virtual_ground_height: {}.",
               virtual_ceil_height, virtual_ground_height);
  }

  /* Rebuild every inflation_step-derived field after changing inflation_step
   * on a copied config (used to instantiate the A* second inflation layer). */
  void recomputeInflationDerived() {
    ...
    /* Restore the yaml-original virtual bounds before resetMapSize: it
     * remaps them in place, so a copied config would otherwise compound
     * the shrink (e.g. ceil 2.0 -> 1.75 -> 1.35) and wrongly occupy
     * legitimate flight altitudes. */
    virtual_ceil_height = virtual_ceil_height_yaml;
    virtual_ground_height = virtual_ground_height_yaml;
    resetMapSize();
  }
};
} // namespace rog_map
