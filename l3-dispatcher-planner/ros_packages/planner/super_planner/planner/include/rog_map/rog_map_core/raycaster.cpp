...
void RayCaster::setResolution(const double& resolution) { resolution_ = resolution; }
...
bool RayCaster::setInput(const Eigen::Vector3d& start, const Eigen::Vector3d& end) {
  ...
  //                std::cout<<"expand_dir_x_:"<<expand_dir_x_<<" expand_dir_y_:"<<expand_dir_y_<<"
  //                expand_dir_z_:"<<expand_dir_z_<<std::endl;
  ...
  t_to_bound_x_ =
      expand_dir_x_ == 0 ? std::numeric_limits<double>::max() : std::fabs(next_bound_x - start_x_d_) / dis_x_over_t;
  t_to_bound_y_ =
      expand_dir_y_ == 0 ? std::numeric_limits<double>::max() : std::fabs(next_bound_y - start_y_d_) / dis_y_over_t;
  t_to_bound_z_ =
      expand_dir_z_ == 0 ? std::numeric_limits<double>::max() : std::fabs(next_bound_z - start_z_d_) / dis_z_over_t;
