...
namespace rog_map {
using super_planner::super_utils::GridType;
...
public:
  ...
  void updateGridCounter(const Vec3f& pos, const GridType& from_type, const GridType& to_type);
...
protected:
  ...
  virtual void triggerJumpingEdge(const Vec3i& id_g, const GridType& from_type, const GridType& to_type) = 0;
  ...
  bool isUnknown(const int& hash_id) const { return md_.unknown_cnt[hash_id] >= md_.unk_thresh; }

  bool isOccupied(const int& hash_id) const { return md_.occupied_cnt[hash_id] > 0; }
