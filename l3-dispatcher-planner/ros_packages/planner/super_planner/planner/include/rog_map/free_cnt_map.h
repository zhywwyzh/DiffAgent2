...
public:
  ...
  FreeCntMap(const Vec3i& half_map_size_i, const double& resolution, const bool& sliding_en,
             const double& sliding_thresh, const Vec3f& fix_map_origin)
      : SlidingMap(half_map_size_i, resolution, sliding_en, sliding_thresh, fix_map_origin) {
  ...
  void resetLocalMap() override {
    SLOG_WARN(" -- [Fro-Map] Clear all local map.");
    std::fill(neighbor_free_cnt.begin(), neighbor_free_cnt.end(), 0);
  }

  int getFreeCnt(const Vec3f& pos) { return neighbor_free_cnt[getHashIndexFromPos(pos)]; }

  int getFreeCnt(const Vec3i& id_g) { return neighbor_free_cnt[getHashIndexFromGlobalIndex(id_g)]; }
  ...
  void resetCell(const int& hash_id) override { neighbor_free_cnt[hash_id] = 0; }
