...
namespace path_search {
    ...
    /* Search on the precomputed A* second inflation layer (O(1) cell query);
     * uses the same index space as the inf map. */
    const int ON_ASTAR_MAP = (1 << 7);
    ...
    class Astar {
        ...
        super_planner::rog_map::vec_Vec3i sorted_pts;
        super_planner::rog_map::vec_Vec3i neighbor_list;
        ...
        struct MissionData {
            super_planner::rog_map::Vec3f start_pt;
            super_planner::rog_map::Vec3f goal_pt;
            double searching_horizon;
            bool use_inf_map{false};
            bool use_prob_map{false};
            bool use_astar_map{false};
            bool unknown_as_occ{false};
            bool unknown_as_free{false};
            bool use_inf_neighbor{false};
            double resolution;
            super_planner::rog_map::Vec3i local_map_center_id_g;
            super_planner::rog_map::Vec3f local_map_center_d;
            double mission_rcv_WT{0};
            super_planner::rog_map::Vec3f local_map_max_d, local_map_min_d;
        ...
        double getHeu(GridNodePtr node1, GridNodePtr node2, int type = DIAG) const;
         ...
         int getLocalIndexHash(const super_planner::rog_map::Vec3i &id_in) const;

        void posToGlobalIndex(const super_planner::rog_map::Vec3f &pos, super_planner::rog_map::Vec3i &id_g) const ;

        void globalIndexToPos(const super_planner::rog_map::Vec3i &id_g, super_planner::rog_map::Vec3f &pos) const;

        bool insideLocalMap(const super_planner::rog_map::Vec3f &pos) const;

        bool insideLocalMap(const super_planner::rog_map::Vec3i &id_g) const;

        bool neighborHaveOne(const super_planner::rog_map::GridType &type, const super_planner::rog_map::Vec3i &src_id);

        RET_CODE setup(const super_planner::rog_map::Vec3f &start_pt, const super_planner::rog_map::Vec3f &goal_pt, const int &flag,
                       const double &searching_horizon = 9999);

        void retrievePath(GridNodePtr current, vector<GridNodePtr> &path);

        void ConvertNodePathToPointPath(const vector<GridNodePtr> &node_path, super_planner::rog_map::vec_Vec3f &point_path);
    ...
    public:
        ...
        RET_CODE pointToPointPathSearch(const super_planner::rog_map::Vec3f &start_pt, const super_planner::rog_map::Vec3f &end_pt,
                                        const int &flag,
        ...
        /// @ param:
        RET_CODE escapePathSearch(const super_planner::rog_map::Vec3f &start_pt, const int flag, super_planner::rog_map::vec_Vec3f &out_path);
