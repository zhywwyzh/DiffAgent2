...
#include "super_utils/tinycolormap.hpp"
...
public:
  ...
  template <typename... Args> static std::string format(const char* fmt, Args&&... args) {
  ...
  // 格式化工具函数
  template <typename... Args> void debug(const char* fmt, Args... args) { debug(format(fmt, args...)); }

  template <typename... Args> void info(const char* fmt, Args... args) { info(format(fmt, args...)); }

  template <typename... Args> void warn(const char* fmt, Args... args) { warn(format(fmt, args...)); }

  template <typename... Args> void error(const char* fmt, Args... args) { error(format(fmt, args...)); }

  template <typename... Args> void fatal(const char* fmt, Args... args) { fatal(format(fmt, args...)); }
  ...
  /*===================Canonical station topics (spec §2.5)==========*/
  /* Publish the front-end A* search result to
   * /drone_<id>/planner/search_path (nav_msgs/Path). */
  virtual void publishSearchPath(const vec_Vec3f& path) = 0;
  /* Publish the committed optimized trajectory to
   * /drone_<id>/planner/optimized_traj (quadrotor_msgs/PiecewisePolynomial,
   * ascending-power coefficients). */
  virtual void publishOptimizedTraj(const Trajectory& committed_traj) = 0;
  ...
  virtual void vizAstarPoints(const Vec3f& position, const Color& c, const std::string& ns, const double& size = 0.1,
                              const int& id = 0) = 0;
  ...
  /* Stream one replan snapshot into the mcap recording (no-op when
   * recording is disabled). keyframe forces the point-cloud write;
   * ambient covers the decimated periodic scene frames. */
  virtual void recordReplan(const Trajectory& exp_traj, const Trajectory& backup_traj, const Trajectory& exp_yaw_traj,
                            const Trajectory& backup_yaw_traj, const PolytopeVec& exp_sfc, const Polytope& backup_sfc,
                            const vec_Vec3f& pc_for_sfc, bool keyframe, bool ambient) {}

  /*Set some parameters==================================*/
  virtual void setResolution(const double& resolution) {
    resolution_ = resolution;
    SLOG_INFO(" -- [Viz] Set resolution_ to {}", resolution_);
  }

  virtual void setVisualizationEn(const bool& en) {
    visualization_en_ = en;
    SLOG_INFO(" -- [Viz] Set visualization_en_ to {}", en);
  }
};
} // namespace ros_interface
} // namespace super_planner
