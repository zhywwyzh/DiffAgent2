...
namespace ros_interface {
    ...
    public:
        ...
        static void addPathToMarkerArray(visualization_msgs::MarkerArray &mkr_ary,
            ...
            if (path.size() <= 0) {
                SLOG_WARN("Try to publish empty path, return.");
                return;
            }
        ...
        static void addTrajectoryToMarkerArray(
            ...
            if (t_sum < 1e-2 || isnan(t_sum) || traj.empty()) {
                SLOG_ERROR("[Trajectory::Visualize] ERROR, the total duration is too small");
                return;
            }
            double eval_t = 0.001;
        ...
        static void addPolytopeToMarkerArray(visualization_msgs::MarkerArray &mkr_arr,
            ...
            if (isnan(planes.sum())) {
                SLOG_ERROR(" -- [Poly] ERROR, try to visualize polytope with NaN, force return.");
                return;
            }
            ...
            if (!super_planner::geometry_utils::enumerateVs(planes, vPoly)) {
                SLOG_WARN(" -- [WARN] Trying to publish ill polytope.");
                return;
            }

            super_planner::geometry_utils::QuickHull<double> tinyQH;
            ...
            auto kLifetime = ros::Duration(0.5);
            ...
            for (long int i = 0; i < planes.rows(); i++) {
                ...
                for (int j = 0; j < temp.size(); j++) {
                    if (std::abs(temp(j)) < super_planner::super_utils::epsilon_) {
        ...
        static void addPointToMarkerArray(visualization_msgs::MarkerArray &mkr_arr,
            ...
            marker_ball.action = visualization_msgs::Marker::ADD;
            marker_ball.lifetime = ros::Duration(0.5);
            marker_ball.pose.orientation.w = 1.0;
            marker_ball.type = visualization_msgs::Marker::SPHERE;
