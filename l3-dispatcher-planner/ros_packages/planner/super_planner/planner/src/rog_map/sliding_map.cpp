...
SlidingMap::SlidingMap(const Vec3i& half_map_size_i, const double& resolution, const bool& map_sliding_en,
  ...
  SLOG_DEBUG("half_map_size_i: {}", half_map_size_i.transpose());
  SLOG_DEBUG("resolution: {}", resolution);
  SLOG_DEBUG("map_sliding_en: {}", map_sliding_en);
  SLOG_DEBUG("sliding_thresh: {}", sliding_thresh);
  SLOG_DEBUG("fix_map_origin: {}", fix_map_origin.transpose());
  initSlidingMap(half_map_size_i, resolution, map_sliding_en, sliding_thresh, fix_map_origin);
}

void SlidingMap::initSlidingMap(const super_planner::rog_map::Vec3i& half_map_size_i, const double& resolution,
                                const bool& map_sliding_en, const double& sliding_thresh,
                                const super_planner::rog_map::Vec3f& fix_map_origin) {
...
void SlidingMap::printMapInformation() {
  const Vec3f map_size_d = sc_.map_size_i.cast<double>() * sc_.resolution;
  SLOG_DEBUG("\tresolution: {}", sc_.resolution);
  SLOG_DEBUG("\tmap_sliding_en: {}", sc_.map_sliding_en);
  SLOG_DEBUG("\tlocal_map_size_i: {}", sc_.map_size_i.transpose());
  SLOG_DEBUG("\tlocal_map_size_d: {}", map_size_d.transpose());
}
...
void SlidingMap::updateLocalMapOriginAndBound(const super_planner::rog_map::Vec3f& new_origin_d,
                                              const super_planner::rog_map::Vec3i& new_origin_i) {
...
void SlidingMap::hashIdToGlobalIndex(const int& hash_id, super_planner::rog_map::Vec3i& id_g) const {
  Vec3i id;
  id(0) = hash_id / (sc_.map_size_i(1) * sc_.map_size_i(2));
  id(1) = (hash_id - id(0) * sc_.map_size_i(1) * sc_.map_size_i(2)) / sc_.map_size_i(2);
  id(2) = hash_id - id(0) * sc_.map_size_i(1) * sc_.map_size_i(2) - id(1) * sc_.map_size_i(2);
