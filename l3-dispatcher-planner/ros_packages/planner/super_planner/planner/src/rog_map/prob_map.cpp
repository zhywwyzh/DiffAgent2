...
void ProbMap::initProbMap() {
  ...
  init_once = true;
  initSlidingMap(cfg_.half_map_size_i, cfg_.resolution, cfg_.map_sliding_en, cfg_.map_sliding_thresh,
                 cfg_.fix_map_origin);
  time_consuming_.resize(7);
  inf_map_ = std::make_shared<InfMap>(cfg_);
  if (cfg_.astar_inflation_step > 0) {
    super_planner::rog_map::Config astar_cfg = cfg_;
    astar_cfg.inflation_step = cfg_.astar_inflation_step;
    astar_cfg.recomputeInflationDerived();
    astar_inf_map_ = std::make_shared<InfMap>(astar_cfg);
  }

  if (cfg_.frontier_extraction_en) {
    fcnt_map_ = std::make_shared<FreeCntMap>(cfg_.half_map_size_i + Vec3i::Constant(2), cfg_.resolution,
                                             cfg_.map_sliding_en, cfg_.map_sliding_thresh, cfg_.fix_map_origin);
  }
  ...
  SLOG_DEBUG("[ProbMap] virtual_ceil_height: {}", cfg_.virtual_ceil_height);
  SLOG_DEBUG("[ProbMap] virtual_ground_height: {}", cfg_.virtual_ground_height);

  if (!cfg_.map_sliding_en) {
    SLOG_INFO(" -- [ProbMap] Map sliding disabled, set origin to [{}] -- ", cfg_.fix_map_origin.transpose());
  ...
  SLOG_INFO(" -- [ProbMap] Init successfully -- .");
  printMapInformation();
}
...
bool ProbMap::isOccupied(const Vec3i& id_g) const {
  ...
  if (id_g.z() > sc_.virtual_ceil_height_id_g || id_g.z() < sc_.virtual_ground_height_id_g + sc_.safe_margin_i) {
    return true;
  }
...
void ProbMap::writeTimeConsumingToLog() {
  SLOG_EVENT(slog::Level::debug, "rog_perf",
             {slog::F("total", time_consuming_[0]), slog::F("raycast", time_consuming_[1]),
              slog::F("update_cache", time_consuming_[2]), slog::F("inflation", time_consuming_[3]),
              slog::F("point_cloud_number", time_consuming_[4]), slog::F("cache_number", time_consuming_[5]),
              slog::F("inflation_number", time_consuming_[6])});
}

void ProbMap::writeMapInfoToLog() {
  SLOG_EVENT(slog::Level::debug, "rog_map_info",
             {slog::F("map_size_d", fmt::format("{}", cfg_.map_size_d.transpose())),
              slog::F("resolution", cfg_.resolution),
              slog::F("map_size_i", fmt::format("{}", sc_.map_size_i.transpose())),
              slog::F("local_update_box_size", fmt::format("{}", cfg_.local_update_box_d.transpose())),
              slog::F("p_min", cfg_.p_min), slog::F("p_max", cfg_.p_max), slog::F("p_hit", cfg_.p_hit),
              slog::F("p_miss", cfg_.p_miss), slog::F("p_occ", cfg_.p_occ), slog::F("p_free", cfg_.p_free),
              slog::F("unk_thresh", cfg_.unk_thresh), slog::F("map_sliding_thresh", cfg_.map_sliding_thresh),
              slog::F("map_sliding_en", cfg_.map_sliding_en),
              slog::F("fix_map_origin", fmt::format("{}", cfg_.fix_map_origin.transpose())),
              slog::F("visualization_range", fmt::format("{}", cfg_.visualization_range.transpose())),
              slog::F("virtual_ceil_height", cfg_.virtual_ceil_height),
              slog::F("virtual_ground_height", cfg_.virtual_ground_height),
              slog::F("batch_update_size", cfg_.batch_update_size),
              slog::F("frontier_extraction_en", cfg_.frontier_extraction_en)});
  inf_map_->writeMapInfoToLog();
}
...
void ProbMap::slideAllMap(const super_planner::rog_map::Vec3f& pos) {
  mapSliding(pos);
  inf_map_->mapSliding(pos);
  if (astar_inf_map_) {
    astar_inf_map_->mapSliding(pos);
  }
  if (cfg_.frontier_extraction_en) {
...
void ProbMap::updateProbMap(const PointCloud& cloud, const Pose& pose) {
  ...
  if (cfg_.map_sliding_en && !insideLocalMap(pos) && raycast_data_.batch_update_counter == 0) {
    SLOG_WARN(" -- [ROGMapCore] cur_pose out of map range, reset the map.");
    SLOG_WARN(" -- [ROGMapCore] Sliding to map center at: {}", pos.transpose());
    slideAllMap(pos);
    return;
  }

  if (pos.z() > cfg_.virtual_ceil_height) {
    SLOG_WARN(" -- [ROGMapCore] Odom above virtual ceil, please check map parameter -- .");
    return;
  } else if (pos.z() < cfg_.virtual_ground_height) {
    SLOG_WARN(" -- [ROGMapCore] Odom below virtual ground, please check map parameter -- .");
    return;
  }
  ...
  /* After the first goal, clear all unknown around the robot once */
  if (pending_clear_unknown_on_first_goal_) {
    pending_clear_unknown_on_first_goal_ = false;
    cleared_unknown_on_first_goal_ = true;
    clearUnknownAroundRobot(pending_clear_pos_);
  }
}

void ProbMap::clearUnknownAroundOnFirstGoal(const Vec3f& pos) {
  if (!cleared_unknown_on_first_goal_) {
    pending_clear_unknown_on_first_goal_ = true;
    pending_clear_pos_ = pos;
  }
}

void ProbMap::clearUnknownAroundRobot(const Vec3f& pos) {
...
GridType ProbMap::getGridType(Vec3i& id_g) const {
  if (id_g.z() <= sc_.virtual_ground_height_id_g || id_g.z() >= sc_.virtual_ceil_height_id_g - sc_.safe_margin_i) {
    return super_planner::super_utils::OCCUPIED;
  }
  if (!insideLocalMap(id_g)) {
    return super_planner::super_utils::OUT_OF_MAP;
  }
  Vec3i id_l;
...
GridType ProbMap::getAstarGridType(const Vec3f& pos) const {
  /* Same boundary semantics as getInfGridType: outside the prob map is
   * OUT_OF_MAP, so the frontend cannot leak past the sliding map. */
  if (!insideLocalMap(pos)) {
    return OUT_OF_MAP;
  }
  if (astar_inf_map_) {
    /* Query by index: the (1+step)*resolution z-margin in the position
     * overload would inflate the virtual ceil/ground twice (once in the
     * config remap, once here). The index path leaves virtual bounds to
     * the prob layer's virtual-occupied cells inflating downward, which
     * is the single-margin semantic the A* clearance layer wants. */
    Vec3i id_g;
    astar_inf_map_->infMapPosToGlobalIndex(pos, id_g);
    return astar_inf_map_->getGridType(id_g);
  }
  return inf_map_->getGridType(pos);
}
...
void ProbMap::boxSearch(const Vec3f& _box_min, const Vec3f& _box_max, const GridType& gt,
  ...
  out_points.clear();
  if (map_empty_) {
    SLOG_WARN(" -- [ROG] Map is empty, cannot perform box search.");
    return;
  }
  if ((_box_max - _box_min).minCoeff() <= 0) {
    SLOG_WARN(" -- [ROG] Box search failed, box size is zero.");
    return;
  }
...
void ProbMap::boundBoxByLocalMap(Vec3f& box_min, Vec3f& box_max) const {
  if ((box_max - box_min).minCoeff() <= 0) {
    box_min = box_max;
    SLOG_WARN("-- [ROG] Bound box is invalid.");
    return;
  }
...
void ProbMap::resetCell(const int& hash_id) {
  ...
  if (isOccupied(ret)) {
    ...
    if (astar_inf_map_) {
      astar_inf_map_->updateGridCounter(pos, OCCUPIED, UNKNOWN);
    }
    ...
    if (astar_inf_map_) {
      astar_inf_map_->updateGridCounter(pos, KNOWN_FREE, UNKNOWN);
    }
    if (cfg_.frontier_extraction_en) {
      Vec3i id_g;
      posToGlobalIndex(pos, id_g);
...
void ProbMap::hitPointUpdate(const Vec3f& pos, const int& hash_id, const int& hit_num) {
  ...
  if (from_type != to_type) {
    ...
    if (astar_inf_map_) {
      astar_inf_map_->updateGridCounter(center_pos, from_type, to_type);
    }
...
void ProbMap::raycastProcess(const PointCloud& input_cloud, const Vec3f& cur_odom) {
  ...
  for (const auto& pcl_p : input_cloud) {
    ...
    // 1.3) filter for virtual ceil and ground glq 这里不合理直接关掉了
...
void ProbMap::resetLocalMap() {
  SLOG_WARN(" -- [Prob-Map] Clear all local map.");
