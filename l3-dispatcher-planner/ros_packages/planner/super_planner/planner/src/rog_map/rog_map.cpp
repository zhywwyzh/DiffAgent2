...
void ROGMap::init() {
  ...
  if (cfg_.load_pcd_en) {
    ...
    if (pcl::io::loadPCDFile(pcd_path, *pcd_map) == -1) {
      SLOG_ERROR("Load pcd file at: [{}] failed!", cfg_.pcd_name);
      exit(-1);
    }
    Pose cur_pose;
    cur_pose.first = Vec3f(0, 0, 0);
    updateOccPointCloud(*pcd_map);
    SLOG_INFO(" -- [ROGMap]Load pcd file success with {} pts.", pcd_map->size());
    map_empty_ = false;
  }
}

bool ROGMap::findNearestCellThat(const bool& is, const GridType& target_type, const Vec3f& start_pos, Vec3f& nearest_pt,
...
bool ROGMap::findNearestInfCellThat(const bool& is, const GridType& target_type, const Vec3f& start_pos,
  ...
  SLOG_WARN(
      " -- [ROGMap] findNearestInfCellThat failed to find all {} neighbors at start_pos: {}, target_type: {}, is: {}",
      cfg_.spherical_neighbor.size(), start_pos.transpose(), target_type, is);
  return false;
}

bool ROGMap::isLineFree(const super_planner::rog_map::Vec3f& start_pt, const super_planner::rog_map::Vec3f& end_pt,
                        const bool& use_inf_map, const bool& use_unk_as_occ) const {
  if (start_pt.array().isNaN().any() || end_pt.array().isNaN().any()) {
    SLOG_WARN(" -- [ROGMap] Call isLineFree with NaN in start or end pt, return false.");
    return false;
  }
...
void ROGMap::updateMap(const PointCloud& cloud, const Pose& pose) {
  ...
  if (cfg_.ros_callback_en) {
    SLOG_WARN("ROS callback is enabled, can not insert map from updateMap API.");
    return;
  }
  ...
  if (cloud.empty()) {
    ...
    if (local_cnt++ > 100) {
      SLOG_WARN("No cloud input, please check the input topic.");
      local_cnt = 0;
    }
    return;
  }
