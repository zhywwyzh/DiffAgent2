...
void InfMap::infMapGlobalIndexToPos(const Vec3i& id_g, Vec3f& pos) const { globalIndexToPos(id_g, pos); }

void InfMap::infMapPosToGlobalIndex(const Vec3f& pos, Vec3i& id) const { posToGlobalIndex(pos, id); }

InfMap::InfMap(super_planner::rog_map::Config& cfg) {
  cfg_ = cfg;
  ...
  initCounterMap(cfg.half_map_size_i, cfg.resolution, cfg.inflation_resolution, max_step, cfg.map_sliding_en,
                 cfg.map_sliding_thresh, cfg.fix_map_origin, cfg.unk_thresh);
  ...
  resetLocalMap();
  cfg_ = cfg;
  SLOG_INFO(" -- [InfMap] Init successfully -- .");
  printMapInformation();
}
...
void InfMap::writeMapInfoToLog() {
  SLOG_EVENT(slog::Level::debug, "rog_inf_map_info",
             {slog::F("resolution", sc_.resolution),
              slog::F("map_size_i", fmt::format("{}", sc_.map_size_i.transpose())),
              slog::F("map_size_d", fmt::format("{}", (sc_.map_size_i.cast<double>() * sc_.resolution).transpose()))});
}

void InfMap::boxSearch(const Vec3f& box_min, const Vec3f& box_max, const GridType& gt, vec_E<Vec3f>& out_points) const {
  out_points.clear();
  if (map_empty_) {
  ...
  if (gt == UNKNOWN) {
    if (!cfg_.unk_inflation_en) {
      out_points.clear();
      SLOG_WARN(" -- [ROG] Unknown inflation is not enabled, cannot perform box search.");
      return;
    }
...
void InfMap::resetLocalMap() {
  SLOG_WARN(" -- [Inf-Map] Clear all local map.");
...
void InfMap::updateUnkInflation(const Vec3i& id_g, const bool is_add) {
  TimeConsuming tc("updateInflation", false);
  if (!cfg_.unk_inflation_en) {
    SLOG_WARN("Cannot updateUnkInflation of InfMap when unk_inflation_en is false.");
    return;
  }
  ...
  for (const auto& nei : cfg_.unk_inf_spherical_neighbor) {
    ...
    if (imd_.unk_inflate_cnt[addr] < 0 || imd_.unk_inflate_cnt[addr] > imd_.unk_neighbor_num) {
      SLOG_ERROR("unk_inflate_cnt: {} unk_neighbor_num: {}", imd_.unk_inflate_cnt[addr], imd_.unk_neighbor_num);
      throw std::runtime_error(" -- [IM] Negative occupancy counter, which should not happened.!");
    }
#endif
  }
...
void InfMap::triggerJumpingEdge(const super_planner::rog_map::Vec3i& id_g,
                                const super_planner::rog_map::GridType& from_type,
                                const super_planner::rog_map::GridType& to_type) {
