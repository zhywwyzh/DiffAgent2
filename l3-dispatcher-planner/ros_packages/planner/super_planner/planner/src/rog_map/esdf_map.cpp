...
namespace rog_map {
void ESDFMap::initESDFMap(const super_planner::rog_map::Vec3i& half_prob_map_size_i, const double& prob_map_resolution,
                          const double& temp_counter_map_resolution,
                          const super_planner::rog_map::Vec3f& local_update_box, const bool& map_sliding_en,
                          const double& sliding_thresh, const super_planner::rog_map::Vec3f& fix_map_origin,
  ...
  initCounterMap(half_prob_map_size_i, prob_map_resolution, temp_counter_map_resolution, 0, map_sliding_en,
                 sliding_thresh, fix_map_origin, unk_thresh);
  ...
  resetLocalMap();
  SLOG_INFO(" -- [ESDFMap] Init successfully -- .");
  printMapInformation();
}

void ESDFMap::getUpdatedBbox(super_planner::rog_map::Vec3f& box_min, super_planner::rog_map::Vec3f& box_max) const {
...
void ESDFMap::resetLocalMap() {
  SLOG_WARN(" -- [ESDFMap] Clear all local map.");
...
double ESDFMap::getDistance(const super_planner::rog_map::Vec3f& pos) const {
  return distance_buffer[getHashIndexFromPos(pos)];
}

double ESDFMap::getDistance(const Vec3i& id_g) const { return distance_buffer[getHashIndexFromGlobalIndex(id_g)]; }

void ESDFMap::updateESDF3D(const super_planner::rog_map::Vec3f& cur_odom) {
  ...
  auto getEsdfLocalIndexHash = [&](const int& id_x, const int& id_y, const int& id_z, bool print = false) {
  ...
  for (int x = min_esdf[0]; x <= max_esdf[0]; x++) {
    for (int y = min_esdf[1]; y <= max_esdf[1]; y++) {
      fillESDF(
          [&](int z) {
            return isOccupied(getEsdfLocalIndexHash(x > mem_end[0] ? x + id_l[0] - sc_.map_size_i[0] : x + id_l[0],
                                                    y > mem_end[1] ? y + id_l[1] - sc_.map_size_i[1] : y + id_l[1], z))
                       ? 0
  ...
  for (int x = min_esdf[0]; x <= max_esdf[0]; x++) {
    for (int z = min_esdf[2]; z <= max_esdf[2]; z++) {
      fillESDF(
          [&](int y) {
            return tmp_buffer1_[getEsdfLocalIndexHash(x > mem_end[0] ? x + id_l[0] - sc_.map_size_i[0] : x + id_l[0], y,
                                                      z > mem_end[2] ? z + id_l[2] - sc_.map_size_i[2] : z + id_l[2])];
          },
          [&](int y, double val) {
            tmp_buffer2_[getEsdfLocalIndexHash(x > mem_end[0] ? x + id_l[0] - sc_.map_size_i[0] : x + id_l[0], y,
  ...
  for (int y = min_esdf[1]; y <= max_esdf[1]; y++) {
    for (int z = min_esdf[2]; z <= max_esdf[2]; z++) {
      fillESDF(
          [&](int x) {
            return tmp_buffer2_[getEsdfLocalIndexHash(x, y > mem_end[1] ? y + id_l[1] - sc_.map_size_i[1] : y + id_l[1],
                                                      z > mem_end[2] ? z + id_l[2] - sc_.map_size_i[2] : z + id_l[2])];
          },
          [&](int x, double val) {
            distance_buffer[getEsdfLocalIndexHash(x, y > mem_end[1] ? y + id_l[1] - sc_.map_size_i[1] : y + id_l[1],
                                                  z > mem_end[2] ? z + id_l[2] - sc_.map_size_i[2] : z + id_l[2])] =
  ...
  for (int y = min_esdf[1]; y <= max_esdf[1]; y++) {
    for (int z = min_esdf[2]; z <= max_esdf[2]; z++) {
      fillESDF(
          ...
          [&](int x, double val) {
            tmp_buffer1_[getEsdfLocalIndexHash(x, y > mem_end[1] ? y + id_l[1] - sc_.map_size_i[1] : y + id_l[1],
                                               z > mem_end[2] ? z + id_l[2] - sc_.map_size_i[2] : z + id_l[2])] =
                sc_.resolution * std::sqrt(val);
          },
          min_esdf[0], max_esdf[0], 0, id_l[0]);
  ...
  double dt = up_t.stop();
  if (dt > 0.1) {
    SLOG_WARN("updateESDF3D time: {}", dt);
  }
  ...
  t += up_t.stop();
  cnt++;
  SLOG_DEBUG("Average time per point: {}us", t / cnt / sc_.map_vox_num * 1e6);
#endif
}

void ESDFMap::resetOneCell(const int& hash_id) {}

void ESDFMap::getPositiveESDFPointCloud(const super_planner::rog_map::Vec3f& box_min_d,
                                        const super_planner::rog_map::Vec3f& box_max_d, const double& visualize_z,
...
void ESDFMap::getNegativeESDFPointCloud(const super_planner::rog_map::Vec3f& box_min_d,
                                        const super_planner::rog_map::Vec3f& box_max_d, const double& visualize_z,
                                        pcl::PointCloud<pcl::PointXYZI>& pcl_pc) {
  std::lock_guard<std::mutex> lck(update_esdf_mtx);
  pcl_pc.clear();
...
void ESDFMap::interpolateTrilinearFirstGrad(double values[2][2][2], const Eigen::Vector3d& diff,
                                            Eigen::Vector3d& grad) {
  // trilinear interpolation
  double v00 = (1 - diff(0)) * values[0][0][0] + diff(0) * values[1][0][0]; // b
  double v01 = (1 - diff(0)) * values[0][0][1] + diff(0) * values[1][0][1]; // d
...
void ESDFMap::interpolateTrilinearSecondGrad(double first_grad[2][2][2][3], const Eigen::Vector3d& diff,
                                             Eigen::Vector3d& grad) {
