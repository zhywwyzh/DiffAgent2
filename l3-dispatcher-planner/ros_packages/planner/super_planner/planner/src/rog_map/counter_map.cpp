...
void CounterMap::initCounterMap(
    ...
    const double& prob_map_resolution, const double& temp_counter_map_resolution, const int& inflation_step,
    const bool& map_sliding_en, const double& sliding_thresh, const Vec3f& fix_map_origin, const double& unk_thresh) {
  ...
  /* 1) compute the resolution ratio, ensure the resolution is an integer (for CORNER) or odd numbered multiple (for
   * CENTER)
   * */
  ...
  SLOG_DEBUG(" -- [CounterMap] inflation_ratio: {}", inflation_ratio);
#endif
  ...
  Vec3i half_counter_map_size_i =
      (half_prob_map_size_d / counter_map_resolution).cast<int>() + (inflation_step + 1) * Vec3i::Ones();
  ...
  resetLocalMap();
  SLOG_INFO(" -- [CounterMap] Init successfully -- .");
  printMapInformation();
}
...
void CounterMap::updateGridCounter(const Vec3f& pos, const GridType& from_type, const GridType& to_type) {
  ...
  if (md_.occupied_cnt[addr] < 0 || md_.occupied_cnt[addr] > md_.sub_grid_num) {
    SLOG_ERROR("From type: {}", GridTypeStr[from_type]);
    SLOG_ERROR("To type: {}", GridTypeStr[to_type]);
    SLOG_ERROR("Occupied counter: {}", md_.occupied_cnt[addr]);
  ...
  if (md_.unknown_cnt[addr] < 0 || md_.unknown_cnt[addr] > md_.sub_grid_num) {
    SLOG_ERROR("From type: {}", from_type);
    SLOG_ERROR("To type: {}", to_type);
    SLOG_ERROR("Unknown counter: {}", md_.unknown_cnt[addr]);
...
bool CounterMap::isUnknown(const Vec3i& id_g) const { return isUnknown(getHashIndexFromGlobalIndex(id_g)); }

bool CounterMap::isOccupied(const Vec3i& id_g) const { return isOccupied(getHashIndexFromGlobalIndex(id_g)); }

bool CounterMap::isKnownFree(const Vec3i& id_g) const { return isKnownFree(getHashIndexFromGlobalIndex(id_g)); }
