...
bool Trajectory::getPartialTrajectoryByTime(const double &start_TT, const double &end_TT,
    ...
    if (start_TT < 0 || start_TT >= total_dur) {
        SLOG_WARN("Partial traj end_t error. [Start_TT]: {} [Total Dur]: {}", start_TT, total_dur);
    }
    if (end_TT <= 0 || end_TT > total_dur) {
        SLOG_WARN("Partial traj end_t error. [end_TT]: {} [Total Dur]: {}", end_TT, total_dur);
        return false;
    }
    if (end_TT <= start_TT) {
        SLOG_WARN("Time duration wrong: start_t: {}; end_t: {}", start_TT, end_TT);
        return false;
    }
...
//            print("pieceIdx = {}, t0 = {}.\n",pieceIdx,t0);
    ...
    if (pieces[pieceIdx].getDegree() == 5) {
        ...
        SLOG_ERROR("[getPartialTrajectory] ERROR, the piece degree is neither 5 n or 7");
...
void Trajectory::printProfile() const {
    SLOG_INFO("[Trajectory::printProfile] The trajectory has {} pieces", getPieceNum());
    SLOG_INFO("\tMaximum Velocity Rate: {}", getMaxVelRate());
    SLOG_INFO("\tMaximum Acceleration Rate: {}", getMaxAccRate());
    SLOG_INFO("\tTotal Duration: {}", getTotalDuration());
}
