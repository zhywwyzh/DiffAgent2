...
double super_planner::geometry_utils::DistancePointEllipse(double e0, double e1,
                            double y0, double y1,
                            double& x0, double& x1) {
    double distance;
...
double
super_planner::geometry_utils::DistancePointEllipsoid(double e0, double e1, double e2,
...
Eigen::Matrix<Scalar_t, 3, 1> super_planner::geometry_utils::quaternion_to_yrp(const Eigen::Quaternion<Scalar_t>& q_) {
...
template Eigen::Matrix<double, 3, 1> super_planner::geometry_utils::quaternion_to_yrp<double>(const Eigen::Quaternion<double>& q_);

template Eigen::Matrix<float, 3, 1> super_planner::geometry_utils::quaternion_to_yrp<float>(const Eigen::Quaternion<float>& q_);


Vec4f super_planner::geometry_utils::translatePlane(const Vec4f& plane, const Vec3f& translation) {
...
void super_planner::geometry_utils::normalizeNextYaw(const double& last_yaw, double& yaw) {
...
void super_planner::geometry_utils::convertFlatOutputToAttAndOmg(const Vec3f& p,
                                                  const Vec3f& v,
                                                  const Vec3f& a,
                                                  const Vec3f& j,
                                                  const double& yaw,
...
bool super_planner::geometry_utils::pointInsidePolytope(const Vec3f& point, const PolyhedronH& polytope,
                                         double margin) {
    Eigen::Vector4d pt_e;
    pt_e.head(3) = point;
    pt_e(3) = 1;
...
double super_planner::geometry_utils::pointLineSegmentDistance(const Vec3f& p, const Vec3f& a, const Vec3f& b) {
    Vec3f ab = b - a;
    Vec3f ap = p - a;
    Vec3f bp = p - b;
    double e = ap.dot(ab);
...
double super_planner::geometry_utils::computePathLength(const vec_E<Vec3f>& path) {
    if (path.size() < 2) {
        return 0.0;
    }
    double len = 0.0;
...
///============ 2022-11-18 ====================================================================================
int super_planner::geometry_utils::GetIntersection(float fDst1, float fDst2, Vec3f P1, Vec3f P2, Vec3f& Hit) {
...
int super_planner::geometry_utils::InBox(Vec3f Hit, Vec3f B1, Vec3f B2, const int Axis) {
...
int super_planner::geometry_utils::lineIntersectBox(Vec3f L1, Vec3f L2, Vec3f B1, Vec3f B2, Vec3f& Hit) {
...
Vec3f super_planner::geometry_utils::lineBoxIntersectPoint(const Vec3f& pt, const Vec3f& pos,
...
Eigen::Matrix3d super_planner::geometry_utils::RotationFromVec3(const Eigen::Vector3d& v) {
    // zero roll
...
// 通过三点获得一个平面
void super_planner::geometry_utils::FromPointsToPlane(const Eigen::Vector3d& p1, const Eigen::Vector3d& p2, const Eigen::Vector3d& p3,
...
void super_planner::geometry_utils::getFovCheckPlane(const Eigen::Matrix3d R, const Eigen::Vector3d t, Eigen::MatrixX4d& fov_planes,
...
void super_planner::geometry_utils::GetFovPlanes(const Eigen::Matrix3d R, const Eigen::Vector3d t, Eigen::MatrixX4d& fov_planes,
                                  std::vector<Eigen::Matrix3d>& fov_pts) {
...
double super_planner::geometry_utils::findInteriorDist(const Eigen::MatrixX4d& hPoly,
...
// Each row of hPoly is defined by h0, h1, h2, h3 as
// h0*x + h1*y + h2*z + h3 <= 0
bool super_planner::geometry_utils::findInterior(const Eigen::MatrixX4d& hPoly,
                                  Eigen::Vector3d& interior) {
    const int m = hPoly.rows();
    ...
    const double minmaxsd = super_planner::math_utils::sdlp::linprog<4>(c, A, b, x);
    interior = x.head<3>();

    return minmaxsd < 0.0 && !std::isinf(minmaxsd);
}

bool super_planner::geometry_utils::overlap(const Eigen::MatrixX4d& hPoly0,
...
void super_planner::geometry_utils::filterVs(const Eigen::Matrix3Xd& rV,
...
void super_planner::geometry_utils::enumerateVs(const Eigen::MatrixX4d& hPoly,
...
// h0*x + h1*y + h2*z + h3 <= 0
// proposed epsilon is 1.0e-6
bool super_planner::geometry_utils::enumerateVs(const Eigen::MatrixX4d& hPoly,
                                 Eigen::Matrix3Xd& vPoly,
                                 const double epsilon) {
...
template <typename Scalar_t>
Scalar_t super_planner::geometry_utils::toRad(const Scalar_t& x) {
    return x / 180.0 * M_PI;
}

template <typename Scalar_t>
Scalar_t super_planner::geometry_utils::toDeg(const Scalar_t& x) {
    return x * 180.0 / M_PI;
}

template <typename Scalar_t>
Eigen::Matrix<Scalar_t, 3, 3> super_planner::geometry_utils::rotx(Scalar_t t) {
...
template <typename Scalar_t>
Eigen::Matrix<Scalar_t, 3, 3> super_planner::geometry_utils::roty(Scalar_t t) {
    Eigen::Matrix<Scalar_t, 3, 3> R;
    R(0, 0) = std::cos(t);
    R(0, 1) = 0.0;
    R(0, 2) = std::sin(t);
...
template <typename Scalar_t>
Eigen::Matrix<Scalar_t, 3, 3> super_planner::geometry_utils::rotz(Scalar_t t) {
    Eigen::Matrix<Scalar_t, 3, 3> R;
    R(0, 0) = std::cos(t);
    R(0, 1) = -std::sin(t);
    R(0, 2) = 0.0;
...
Eigen::Matrix<typename Derived::Scalar, 3, 3> super_planner::geometry_utils::ypr_to_R(const Eigen::DenseBase<Derived>& ypr) {
...
super_planner::geometry_utils::vec_to_R(const Eigen::MatrixBase<Derived>& v1, const Eigen::MatrixBase<Derived>& v2) {
    EIGEN_STATIC_ASSERT_FIXED_SIZE(Derived);
    EIGEN_STATIC_ASSERT(Derived::RowsAtCompileTime == 3, THIS_METHOD_IS_ONLY_FOR_MATRICES_OF_A_SPECIFIC_SIZE);
    EIGEN_STATIC_ASSERT(Derived::ColsAtCompileTime == 1, THIS_METHOD_IS_ONLY_FOR_MATRICES_OF_A_SPECIFIC_SIZE);
...
template <typename Derived>
Eigen::Matrix<typename Derived::Scalar, 3, 1> super_planner::geometry_utils::R_to_ypr(const Eigen::DenseBase<Derived>& R) {
    EIGEN_STATIC_ASSERT_FIXED_SIZE(Derived);
    EIGEN_STATIC_ASSERT(Derived::RowsAtCompileTime == 3, THIS_METHOD_IS_ONLY_FOR_MATRICES_OF_A_SPECIFIC_SIZE);
...
template <typename Derived>
Eigen::Quaternion<typename Derived::Scalar> super_planner::geometry_utils::ypr_to_quaternion(const Eigen::DenseBase<Derived>& ypr) {
    EIGEN_STATIC_ASSERT_FIXED_SIZE(Derived);
    EIGEN_STATIC_ASSERT(Derived::RowsAtCompileTime == 3, THIS_METHOD_IS_ONLY_FOR_MATRICES_OF_A_SPECIFIC_SIZE);
    EIGEN_STATIC_ASSERT(Derived::ColsAtCompileTime == 1, THIS_METHOD_IS_ONLY_FOR_MATRICES_OF_A_SPECIFIC_SIZE);
...
template <typename Scalar_t>
Eigen::Matrix<Scalar_t, 3, 1> super_planner::geometry_utils::quaternion_to_ypr(const Eigen::Quaternion<Scalar_t>& q_) {
    Eigen::Quaternion<Scalar_t> q = q_.normalized();
...
template <typename Scalar_t>
Scalar_t super_planner::geometry_utils::get_yaw_from_quaternion(const Eigen::Quaternion<Scalar_t>& q) {
    return quaternion_to_ypr(q)(0);
}

template <typename Scalar_t>
Eigen::Quaternion<Scalar_t> super_planner::geometry_utils::yaw_to_quaternion(Scalar_t yaw) {
    return Eigen::Quaternion<Scalar_t>(rotz(yaw));
}

template <typename Scalar_t>
Scalar_t super_planner::geometry_utils::normalize_angle(Scalar_t a) {
    int cnt = 0;
    while (true) {
        cnt++;
...
template <typename Scalar_t>
Scalar_t super_planner::geometry_utils::angle_add(Scalar_t a, Scalar_t b) {
    Scalar_t c = a + b;
    c = normalize_angle(c);
...
template <typename Scalar_t>
Scalar_t super_planner::geometry_utils::yaw_add(Scalar_t a, Scalar_t b) {
    return angle_add(a, b);
}

template <typename Derived>
Eigen::Matrix<typename Derived::Scalar, 3, 3> super_planner::geometry_utils::get_skew_symmetric(const Eigen::DenseBase<Derived>& v) {
    EIGEN_STATIC_ASSERT_FIXED_SIZE(Derived);
    EIGEN_STATIC_ASSERT(Derived::RowsAtCompileTime == 3, THIS_METHOD_IS_ONLY_FOR_MATRICES_OF_A_SPECIFIC_SIZE);
    EIGEN_STATIC_ASSERT(Derived::ColsAtCompileTime == 1, THIS_METHOD_IS_ONLY_FOR_MATRICES_OF_A_SPECIFIC_SIZE);
...
template <typename Derived>
Eigen::Matrix<typename Derived::Scalar, 3, 1> super_planner::geometry_utils::from_skew_symmetric(const Eigen::DenseBase<Derived>& M) {
    EIGEN_STATIC_ASSERT_FIXED_SIZE(Derived);
    EIGEN_STATIC_ASSERT(Derived::RowsAtCompileTime == 3, THIS_METHOD_IS_ONLY_FOR_MATRICES_OF_A_SPECIFIC_SIZE);
    EIGEN_STATIC_ASSERT(Derived::ColsAtCompileTime == 3, THIS_METHOD_IS_ONLY_FOR_MATRICES_OF_A_SPECIFIC_SIZE);
...
super_planner::geometry_utils::from_skew_symmetric<Eigen::Matrix3d>(const Eigen::DenseBase<Eigen::Matrix3d>& M);

template Eigen::Matrix<double, 3, 3>
super_planner::geometry_utils::get_skew_symmetric<Eigen::Vector3d>(const Eigen::DenseBase<Eigen::Vector3d>& v);

template double super_planner::geometry_utils::toDeg(const double& x);

template double super_planner::geometry_utils::toRad(const double& x);

template Eigen::Matrix<double, 3, 3> super_planner::geometry_utils::rotx(double t);

template Eigen::Matrix<double, 3, 3> super_planner::geometry_utils::roty(double t);

template Eigen::Matrix<double, 3, 3> super_planner::geometry_utils::rotz(double t);

template Eigen::Matrix<double, 3, 3> super_planner::geometry_utils::ypr_to_R(const Eigen::DenseBase<Eigen::Vector3d>& ypr);

template Eigen::Matrix<double, 3, 3>
super_planner::geometry_utils::vec_to_R(const Eigen::MatrixBase<Eigen::Vector3d>& v1, const Eigen::MatrixBase<Eigen::Vector3d>& v2);

template Eigen::Matrix<double, 3, 1> super_planner::geometry_utils::R_to_ypr(const Eigen::DenseBase<Eigen::Matrix3d>& R);

template Eigen::Quaternion<double> super_planner::geometry_utils::ypr_to_quaternion(const Eigen::DenseBase<Eigen::Vector3d>& ypr);

template Eigen::Matrix<double, 3, 1> super_planner::geometry_utils::quaternion_to_ypr(const Eigen::Quaternion<double>& q_);

template double super_planner::geometry_utils::get_yaw_from_quaternion(const Eigen::Quaternion<double>& q);

template Eigen::Quaternion<double> super_planner::geometry_utils::yaw_to_quaternion(double yaw);

template double super_planner::geometry_utils::normalize_angle(double a);

template double super_planner::geometry_utils::angle_add(double a, double b);

template double super_planner::geometry_utils::yaw_add(double a, double b);


template Eigen::Matrix<float, 3, 1>
super_planner::geometry_utils::from_skew_symmetric<Eigen::Matrix3f>(const Eigen::DenseBase<Eigen::Matrix3f>& M);

template Eigen::Matrix<float, 3, 3>
super_planner::geometry_utils::get_skew_symmetric<Eigen::Vector3f>(const Eigen::DenseBase<Eigen::Vector3f>& v);

template float super_planner::geometry_utils::toDeg(const float& x);

template float super_planner::geometry_utils::toRad(const float& x);

template Eigen::Matrix<float, 3, 3> super_planner::geometry_utils::rotx(float t);

template Eigen::Matrix<float, 3, 3> super_planner::geometry_utils::roty(float t);

template Eigen::Matrix<float, 3, 3> super_planner::geometry_utils::rotz(float t);

template Eigen::Matrix<float, 3, 3> super_planner::geometry_utils::ypr_to_R(const Eigen::DenseBase<Eigen::Vector3f>& ypr);

template Eigen::Matrix<float, 3, 3>
super_planner::geometry_utils::vec_to_R(const Eigen::MatrixBase<Eigen::Vector3f>& v1, const Eigen::MatrixBase<Eigen::Vector3f>& v2);

template Eigen::Matrix<float, 3, 1> super_planner::geometry_utils::R_to_ypr(const Eigen::DenseBase<Eigen::Matrix3f>& R);

template Eigen::Quaternion<float> super_planner::geometry_utils::ypr_to_quaternion(const Eigen::DenseBase<Eigen::Vector3f>& ypr);

template Eigen::Matrix<float, 3, 1> super_planner::geometry_utils::quaternion_to_ypr(const Eigen::Quaternion<float>& q_);

template float super_planner::geometry_utils::get_yaw_from_quaternion(const Eigen::Quaternion<float>& q);

template Eigen::Quaternion<float> super_planner::geometry_utils::yaw_to_quaternion(float yaw);

template float super_planner::geometry_utils::normalize_angle(float a);

template float super_planner::geometry_utils::angle_add(float a, float b);

template float super_planner::geometry_utils::yaw_add(float a, float b);
