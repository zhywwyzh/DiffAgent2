...
double Piece::getMaxVelRate() const {
    ...
    Eigen::VectorXd coeff = super_planner::math_utils::RootFinder::polySqr(nVelCoeffMat.row(0)) +
                            super_planner::math_utils::RootFinder::polySqr(nVelCoeffMat.row(1)) +
                            super_planner::math_utils::RootFinder::polySqr(nVelCoeffMat.row(2));
    int N = coeff.size();
    int n = N - 1;
    for (int i = 0; i < N; i++) {
        coeff(i) *= n;
...
//        print("begin solve poly. {}\n", coeff.head(N - 1).squaredNorm());
    if (coeff.head(N - 1).squaredNorm() < 1e-6) {
        ...
        double r = 1.0625;
        int cnt = 0;
        while (fabs(super_planner::math_utils::RootFinder::polyVal(coeff.head(N - 1), l)) < DBL_EPSILON) {
            l = 0.5 * l;
            if (cnt++ > 100) {
                SLOG_WARN("super_planner::math_utils::RootFinder stuck in inf-loop");
            }
        }
        cnt = 0;
        while (fabs(super_planner::math_utils::RootFinder::polyVal(coeff.head(N - 1), r)) < DBL_EPSILON) {
            r = 0.5 * (r + 1.0);
            if (cnt++ > 100) {
                SLOG_WARN("super_planner::math_utils::RootFinder stuck in inf-loop");
            }
        }
        std::set<double> candidates = super_planner::math_utils::RootFinder::solvePolynomial(coeff.head(N - 1), l, r,
...
double Piece::getMaxAccRate() const {
    ...
    Eigen::VectorXd coeff = super_planner::math_utils::RootFinder::polySqr(nAccCoeffMat.row(0)) +
                            super_planner::math_utils::RootFinder::polySqr(nAccCoeffMat.row(1)) +
                            super_planner::math_utils::RootFinder::polySqr(nAccCoeffMat.row(2));
    int N = coeff.size();
    int n = N - 1;
    for (int i = 0; i < N; i++) {
        coeff(i) *= n;
...
bool Piece::checkMaxVelRate(const double &maxVelRate) const {
    ...
    if (getVel(0.0).squaredNorm() >= sqrMaxVelRate ||
        ...
        return super_planner::math_utils::RootFinder::countRoots(coeff, 0.0, 1.0) == 0;
    }
}
