...
Vec3f Polytope::CrossCenter(const Polytope &b) const {
    ...
    Mat3Df curIV; // 走廊的顶点
    if (!super_planner::geometry_utils::enumerateVs(curIH, curIV)) {
...
bool Polytope::HaveOverlapWith(Polytope cmp, double eps) {
    return super_planner::geometry_utils::overlap(this->planes, cmp.GetPlanes(), eps);
}
...
bool Polytope::PointIsInside(const Vec3f &pt, const double & margin) const {
    ...
    if (planes.rows() == 0 || isnan(planes.sum())) {
        SLOG_WARN("ill polytope, force return.");
    }
    Eigen::Vector4d pt_e;
    pt_e.head(3) = pt;
    pt_e(3) = 1;
...
double Polytope::GetVolume() const {
    ...
    if (!super_planner::geometry_utils::enumerateVs(planes, vPoly)) {
        SLOG_WARN("Failed to compute volume: cannot enumerate vertices.");
        return 0;
    }

    // 使用QuickHull库计算凸包
    super_planner::geometry_utils::QuickHull<double> qh;
    ...
    if (indexBuffer.size() < 4) {
        SLOG_WARN("Not enough vertices to compute volume.");
        return 0;
    }
