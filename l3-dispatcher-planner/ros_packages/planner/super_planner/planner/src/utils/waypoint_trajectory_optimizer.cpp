...
//            print(fg(color::yellow), "MINCO Internal cost {}\n", cost);
    ...
    void GcopterWayptS4::setInitial() {
        ...
        if (waypoints.size() == 0) {
            ...
            if (delta.norm() == 0) {
                SLOG_WARN("delta norm is zero");
                SLOG_DEBUG("{}", free_points.rightCols(1).transpose());
                SLOG_DEBUG("{}", points.rightCols(1).transpose());
                SLOG_DEBUG("{}", tailPVAJ.col(0).transpose());
                SLOG_WARN("delta norm is zero");
                SLOG_DEBUG("{}", waypoints.transpose());
            }
            times(temporalDim - 1) = delta.norm() / allocationSpeed;
        }
        return;
