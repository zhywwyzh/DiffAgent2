...
//            cfg_.penna_thr;
    ...
    const double vmaxZSqr = magnitudeBounds.size() > 6 && magnitudeBounds[6] > 0
...
bool BackupTrajOpt::processCorridor() {
    ...
    if (!super_planner::geometry_utils::enumerateVs(opt_vars.hPolytope, curIV)) {
        SLOG_WARN(" -- [MINCO] enumerateVs failed.");
        return false;
    }
    long nv = curIV.cols();
    curIOB.resize(3, nv);
...
bool BackupTrajOpt::setupProblemAndCheck() {
    ...
    if (!processCorridor()) {
        SLOG_WARN(" -- [processCorridor] Failed to get Overlap enumerateVs .");
        return false;
    }
...
BackupTrajOpt::BackupTrajOpt(const super_planner::traj_opt::Config &cfg, const super_planner::ros_interface::RosInterface::Ptr &ros_ptr)
    ...
    opt_vars.magnitudeBounds.resize(7);
...
bool BackupTrajOpt::checkTrajMagnitudeBound(Trajectory &out_traj) {
    if (cfg_.penna_vel > 0 && out_traj.getMaxVelRate() > 1.2 * cfg_.max_vel) {
        SLOG_WARN(" -- [TrajOpt] Minco backup opt failed.");
        SLOG_WARN("\t\tBackend Max vel:\t{} m/s", out_traj.getMaxVelRate());
        return false;
    }
    if (cfg_.penna_acc > 0 && out_traj.getMaxAccRate() > 1.2 * cfg_.max_acc) {
        SLOG_WARN(" -- [TrajOpt] Minco backup opt failed.");
        SLOG_WARN("\t\tBackend Max Acc:\t{} m/s", out_traj.getMaxAccRate());
        return false;
    }
    return true;
}
...
BackupTrajOpt::optimize(const Trajectory &exp_traj,
    ...
    if (std::isnan(opt_vars.hPolytope.sum())) {
        SLOG_WARN(" -- [BackTrajOpt] Polytope is nan.");
        return false;
    }
    ...
    if (!setupProblemAndCheck()) {
        SLOG_WARN(" -- [TrajOpt] Minco corridor preprocess error.");
        success = false;
    }

    if (success && std::isinf(optimize(out_traj, cfg_.opt_accuracy))) {
        SLOG_WARN(" -- [SUPER] Minco backup_traj opt failed.");
        success = false;
    }

    if (opt_vars.penalty_log(1) > cfg_.penna_pos * 0.05) {
        SLOG_WARN(" -- [SUPER] Minco backup_traj out of corridor.");
        success = false;
    }
    out_ts = opt_vars.ts;
