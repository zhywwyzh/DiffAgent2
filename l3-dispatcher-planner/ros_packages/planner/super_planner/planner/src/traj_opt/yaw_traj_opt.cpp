...
//                          " -- [SUPER] Yaw planning failed, the goal yaw is too close to the current yaw.\n");
        ...
        if (way_pts.size() == 0) {
            super_planner::geometry_utils::normalizeNextYaw(init_state[0], goal_state[0]);
        } else {
            super_planner::geometry_utils::normalizeNextYaw(way_pts(way_pts.size() - 1), goal_state[0]);
        }
...
//            cout << times.transpose() << endl;
    ...
    bool YawTrajOpt::optimize(const Vec4f &istate_in,
        ...
        switch (order) {
            case 3: {
                const super_planner::super_utils::Vec2f init_state3 = init_state.head(2);
                const super_planner::super_utils::Vec2f goal_state3 = goal_state.head(2);
            ...
            default: {
                SLOG_ERROR("Unsupported order for yaw trajectory optimization.");
                return false;
            }
        }
...
//            yaw_traj.printProfile();
        ...
        if (max_yaw_rate > yaw_dot_max_ + 2.0) {
            SLOG_WARN(" Yaw rate too large, {}", max_yaw_rate);
//                return false;
        }
        out_traj = yaw_traj;
