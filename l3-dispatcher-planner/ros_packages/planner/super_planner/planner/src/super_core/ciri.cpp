...
//                        infeasible_problem = true;
                        ...
                        SLOG_WARN(" -- [CIRI] WARNING! The problem is not feasible, the min dis to obstacle is only: {}", dis);
                        return FAILED;
                        SLOG_WARN(" -- [CIRI] dis: {}", dis);
                        SLOG_WARN(" -- [CIRI] robot_r: {}", robot_r_);
                        SLOG_WARN(" -- [CIRI] pcMin: {}", pt_w.transpose());
                        SLOG_WARN(" -- [CIRI] a: {}", a.transpose());
                        SLOG_WARN(" -- [CIRI] b: {}", b.transpose());
                    }
            ...
            if(hPoly.array().isNaN().any()) {
                SLOG_WARN(" -- [CIRI] ERROR! maxVolInsEllipsoid failed.");
...
//                std::cout << " -- [CIRI] hPoly: " << hPoly << std::endl;
        ...
        if (std::isnan(hPoly.sum())) {
            SLOG_WARN(" -- [CIRI] ERROR! There is nan in generated planes.");
            SLOG_WARN("{}", a.transpose());
            SLOG_WARN("{}", b.transpose());
            ros_ptr_->vizCiriSeedLine(a, b,robot_r_);
            ros_ptr_->vizCiriEllipsoid(E);
            return FAILED;
        }
        Vec3f inner;
        if (!super_planner::geometry_utils::findInterior(hPoly, inner)) {
            SLOG_WARN(" -- [CIRI] The polytope is empty.");
            ros_ptr_->vizCiriSeedLine(a, b,robot_r_);
            ros_ptr_->vizCiriEllipsoid(E);
            return FAILED;
        }
    ...
    void CIRI::findEllipsoid(const Eigen::Matrix3Xd& pc,
        ...
        if (max_iter == 0) {
            SLOG_WARN(" -- [CIRI] Find Ellipsoid reach max iteration, may cause error.");
        }
        max_iter = 100;
