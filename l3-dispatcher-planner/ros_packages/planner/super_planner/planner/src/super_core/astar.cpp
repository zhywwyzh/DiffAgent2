...
using namespace super_planner::rog_map;

Astar::Astar(const std::string &cfg_path,
             const super_planner::ros_interface::RosInterface::Ptr &ros_ptr,
             super_planner::rog_map::ROGMap::Ptr rm)
    : ros_ptr_(ros_ptr), map_ptr_(rm) {
  cfg_ = PathSearchConfig(cfg_path);
  tie_breaker_ = cfg_.tie_breaker;
  SLOG_DEBUG(" -- [RM] Init Astar-map.");
  SLOG_DEBUG("\tz_cost_penalty: {}, tie_breaker: {}", cfg_.z_cost_penalty,
            tie_breaker_);
  int map_buffer_size =
      cfg_.map_voxel_num(0) * cfg_.map_voxel_num(1) * cfg_.map_voxel_num(2);
  grid_node_buffer_.resize(map_buffer_size);
  for (auto &i : grid_node_buffer_) {
    i = new GridNode;
    i->rounds = 0;
  }
  SLOG_DEBUG("\tmap index size: {}", cfg_.map_size_i.transpose());
  SLOG_DEBUG("\tmap vox_num: {}", cfg_.map_voxel_num.transpose());
  int test_num = 100;
  for (int i = -test_num; i <= test_num; i++) {
    for (int j = -test_num; j <= test_num; j++) {
      for (int k = -test_num; k <= test_num; k++) {
        super_planner::rog_map::Vec3i delta(i, j, k);
        sorted_pts.push_back(delta);
      }
    }
  }
  sort(sorted_pts.begin(), sorted_pts.end(),
       [](const super_planner::rog_map::Vec3i &pt1, const super_planner::rog_map::Vec3i &pt2) {
         double dist1 =
...
RET_CODE
Astar::setup(const Vec3f &start_pt, const Vec3f &goal_pt, const int &flag,
  ...
  md_.use_astar_map = flag & ON_ASTAR_MAP;
  ...
  int map_kind_count = (md_.use_inf_map ? 1 : 0) + (md_.use_prob_map ? 1 : 0) +
                       (md_.use_astar_map ? 1 : 0);
  if (map_kind_count != 1) {
    SLOG_ERROR(" -- [A*] {}: must select exactly one map layer.",
               RET_CODE_STR[INIT_ERROR]);
    return INIT_ERROR;
  }
  if (md_.unknown_as_occ && md_.unknown_as_free) {
    SLOG_ERROR(" -- [A*] {}: cannot use both unknown_as_occupied and "
               "unknown_as_free.",
               RET_CODE_STR[INIT_ERROR]);
    return INIT_ERROR;
  }
  ...
  md_.local_map_min_d =
      md_.local_map_center_d - md_.resolution * cfg_.map_size_i.cast<double>();
  md_.local_map_max_d =
      md_.local_map_center_d + md_.resolution * cfg_.map_size_i.cast<double>();
  ;
...
double Astar::getHeu(GridNodePtr node1, GridNodePtr node2, int type) const {
  ...
  case DIAG: {
    ...
    if (dx == 0) {
      h = 1.0 * sqrt(3.0) * diag + sqrt(2.0) * std::min(dy, dz) +
          1.0 * std::abs(dy - dz);
    }
    if (dy == 0) {
      h = 1.0 * sqrt(3.0) * diag + sqrt(2.0) * std::min(dx, dz) +
          1.0 * std::abs(dx - dz);
    }
    if (dz == 0) {
      h = 1.0 * sqrt(3.0) * diag + sqrt(2.0) * std::min(dx, dy) +
          1.0 * std::abs(dx - dy);
    }
  ...
  case EUCL: {
    ...
    return tie_breaker_ *
           sqrt(d.x() * d.x() + d.y() * d.y() + zp * zp * d.z() * d.z());
  }
  case XY_OCTILE_MANH_Z: {
    ...
    const double zp = cfg_.z_cost_penalty;
    return tie_breaker_ *
           (std::max(dx, dy) + (sqrt(2.0) - 1.0) * std::min(dx, dy) + zp * dz);
  }
  default: {
    SLOG_ERROR(" -- [A*] Wrong hue type.");
    return 0;
  }
  }
}
...
int Astar::getLocalIndexHash(const Vec3i &id_in) const {
  super_planner::rog_map::Vec3i id = id_in - md_.local_map_center_id_g + cfg_.map_size_i;
  return id(0) * cfg_.map_voxel_num(1) * cfg_.map_voxel_num(2) +
         id(1) * cfg_.map_voxel_num(2) + id(2);
}

void Astar::posToGlobalIndex(const super_planner::rog_map::Vec3f &pos,
                             super_planner::rog_map::Vec3i &id_g) const {
  if (md_.use_inf_map || md_.use_astar_map) {
...
void Astar::globalIndexToPos(const super_planner::rog_map::Vec3i &id_g,
                             super_planner::rog_map::Vec3f &pos) const {
  if (md_.use_inf_map || md_.use_astar_map) {
...
bool Astar::insideLocalMap(const super_planner::rog_map::Vec3f &pos) const {
  super_planner::rog_map::Vec3i id_g;
  posToGlobalIndex(pos, id_g);
  return insideLocalMap(id_g);
}

bool Astar::insideLocalMap(const super_planner::rog_map::Vec3i &id_g) const {
  super_planner::rog_map::Vec3i delta = id_g - md_.local_map_center_id_g;
...
void Astar::setVisualProcessEn(const bool &en) { cfg_.visual_process = en; }
...
void Astar::ConvertNodePathToPointPath(const vector<GridNodePtr> &node_path,
                                       super_planner::rog_map::vec_Vec3f &point_path) {
  point_path.clear();
  for (auto ptr : node_path) {
    super_planner::rog_map::Vec3f pos;
...
RET_CODE Astar::pointToPointPathSearch(const super_planner::rog_map::Vec3f &start_pt,
                                       const super_planner::rog_map::Vec3f &end_pt,
                                       const int &flag,
                                       const double &searching_horizon,
                                       super_planner::rog_map::vec_Vec3f &out_path,
  ...
  super_planner::rog_map::Vec3f hit_pt;
  super_planner::rog_map::Vec3f local_start_pt, local_end_pt;
  ...
  if (!insideLocalMap(start_pt)) {
    ros_ptr_->warn(" -- [A*] Start point [{}] is out of local map, find a "
                   "waypoint to the map edge.",
                   start_pt.transpose());
    if (super_planner::rog_map::lineIntersectBox(start_pt, md_.local_map_center_d,
                                  md_.local_map_min_d, md_.local_map_max_d,
                                  hit_pt)) {
      super_planner::rog_map::Vec3f dir = (hit_pt - start_pt).normalized();
      ...
      if (!map_ptr_->getNearestInfCellNot(OCCUPIED, local_start_pt,
        ...
        if (cfg_.visual_process || cfg_.debug_visualization_en) {
          ros_ptr_->vizAstarPoints(local_start_pt, Color::Orange(),
                                   "local_start_pt", 0.3, 1);
        }
        SLOG_WARN(" -- [A*] {} : start point deeply occupied, cannot find "
                  "feasible path.",
                  RET_CODE_STR[INIT_ERROR]);
        return INIT_ERROR;
      }
  ...
  if (!insideLocalMap(end_pt)) {
    super_planner::rog_map::Vec3f seed_pt =
        start_pt_out_local_map ? md_.local_map_center_d : start_pt;
    if (super_planner::rog_map::lineIntersectBox(end_pt, seed_pt, md_.local_map_min_d,
                                  md_.local_map_max_d, hit_pt)) {
      super_planner::rog_map::Vec3f dir = (hit_pt - end_pt).normalized();
      double dis = (hit_pt - end_pt).norm();
      local_end_pt = end_pt + dir * (dis + 2.5);

      if (!map_ptr_->getNearestInfCellNot(OCCUPIED, local_end_pt, local_end_pt,
                                          2.0)) {
        ros_ptr_->error(" -- [A*] Error with: {}, Goal point [{}] deeply "
                        "occupied, cannot find feasible path.",
                        RET_CODE_STR[INIT_ERROR], local_end_pt.transpose());
        if (cfg_.visual_process || cfg_.debug_visualization_en) {
  ...
  super_planner::rog_map::Vec3i start_idx, end_idx;
  ...
  if (!insideLocalMap(start_idx) || !insideLocalMap(end_idx)) {
    SLOG_ERROR(" -- [RM] Start or end point is out of local map, which "
               "should not happen.");
    ros_ptr_->error(" -- [RM] Start [{}] or end point [{}] is out of local "
                    "map, which should not happen.",
                    local_start_pt.transpose(), local_end_pt.transpose());
    if (cfg_.visual_process || cfg_.debug_visualization_en) {
      ros_ptr_->vizAstarPoints(local_start_pt, Color::Orange(),
                               "local_start_pt", 0.3, 1);
      ros_ptr_->vizAstarPoints(local_end_pt, Color::Green(), "local_end_pt",
                               0.3, 1);
    }
    return INIT_ERROR;
  }
  ...
  std::priority_queue<GridNodePtr, std::vector<GridNodePtr>, NodeComparator>
      open_set;
  std::priority_queue<GridNodePtr, std::vector<GridNodePtr>, FrontierComparator>
      frontier_queue;
  ...
  if (cfg_.visual_process) {
    ros_ptr_->vizAstarPoints(start_pt, Color::Green(), "start_pt", 0.3, 1);
    ros_ptr_->vizAstarPoints(end_pt, Color::Blue(), "goal_pt", 0.3, 1);
  }
  ...
  while (!open_set.empty()) {
    ...
    open_set.pop();
    if (cfg_.visual_process) {
      super_planner::rog_map::Vec3f local_pt;
      globalIndexToPos(current->id_g, local_pt);
      ros_ptr_->vizAstarPoints(local_pt, Color(Color::Pink(), 0.5),
                               "astar_process", 0.1);
      usleep(1000);
    }
    if (current->id_g(0) == endPtr->id_g(0) &&
      ...
      if (start_pt_out_local_map) {
        super_planner::rog_map::Vec3i start_idx_g;
    ...
    if (searching_horizon > 0 &&
        current->distance_score > searching_horizon / md_.resolution) {
    ...
    for (int dx = -1; dx <= 1; dx++)
      for (int dy = -1; dy <= 1; dy++)
        for (int dz = -1; dz <= 1; dz++) {
          ...
          super_planner::rog_map::Vec3i neighborIdx;
          super_planner::rog_map::Vec3f neighborPos;
          ...
          super_planner::rog_map::GridType neighbor_type;

          if (md_.use_astar_map) {
            /* O(1) hard constraint on the precomputed A* second
             * inflation layer (no runtime neighbor dilation). */
            neighbor_type = map_ptr_->getAstarGridType(neighborPos);
          } else if (md_.use_inf_map) {
            ...
            if (!md_.use_inf_neighbor) {
              ...
              // if there is one neighbor is occupied, then the neighbor is
              // occupied.
              neighbor_type =
                  neighborHaveOne(OCCUPIED, neighborIdx) ? OCCUPIED : UNDEFINED;
              // if there is one known free neighbor, then the neighbor is known
              // free.
              if (md_.unknown_as_occ && neighbor_type != OCCUPIED) {
                neighbor_type = neighborHaveOne(KNOWN_FREE, neighborIdx)
                                    ? KNOWN_FREE
                                    : UNKNOWN;
              }
            }
          ...
          if (neighborPtr == nullptr) {
            SLOG_ERROR(" -- [RM] neighborPtr is null, which should not "
                       "happen.");
            continue;
          }
          ...
          if (md_.unknown_as_occ && neighbor_type == UNKNOWN && neighborPtr) {
            ...
            neighborPtr->distance_to_goal =
                getHeu(neighborPtr, endPtr, cfg_.heu_type);
    ...
    if (!cfg_.visual_process && (time_2 - time_1) > time_out) {
      SLOG_WARN("Failed in A star path searching !!! {} seconds time limit "
                "exceeded.",
                time_out);
      return TIME_OUT;
    }
  ...
  if ((time_2 - time_1) > time_out) {
    SLOG_WARN("Time consume in A star path finding is {} s, iter={}.",
  ...
  if (md_.unknown_as_occ && !frontier_queue.empty()) {
    ...
    if (frontier_queue.empty()) {
      SLOG_WARN(" -- [A*] Frontier queue is empty, return.");
      return NO_PATH;
    }
    retrievePath(local_goal, node_path);
    if (start_pt_out_local_map) {
    ...
    ConvertNodePathToPointPath(node_path, out_path);
    SLOG_DEBUG("Frontier queue: {}", frontier_queue.size());
    return REACH_HORIZON;
  }
  ros_ptr_->error(" -- [A*] Point to point path cannot find path with iter "
                  "num: {}, return.",
                  num_iter);
  return NO_PATH;
}

RET_CODE Astar::escapePathSearch(const super_planner::rog_map::Vec3f &start_pt, const int flag,
                                 super_planner::rog_map::vec_Vec3f &out_path) {
  Vec3f tmp;
  ...
  if (!insideLocalMap(start_pt) || !map_ptr_->insideLocalMap(start_pt)) {
    SLOG_WARN(" -- [A*] {}: escape start point is not inside local map.",
              RET_CODE_STR[INIT_ERROR].c_str());
    return INIT_ERROR;
  }
  super_planner::rog_map::Vec3f local_start_pt = start_pt;
  ...
  if (!map_ptr_->getNearestCellNot(OCCUPIED, start_pt, local_start_pt, 3.0)) {
    SLOG_WARN(" -- [A*] {} : escape start point deeply occupied, cannot find "
              "feasible path.",
              RET_CODE_STR[INIT_ERROR]);
    return INIT_ERROR;
  }

  super_planner::rog_map::Vec3i start_idx;
  posToGlobalIndex(local_start_pt, start_idx);
  ...
  while (!open_set.empty()) {
    ...
    if (cfg_.visual_process) {
      super_planner::rog_map::Vec3f local_pt;
      globalIndexToPos(current->id_g, local_pt);
      ros_ptr_->vizAstarPoints(local_pt, Color::Pink(), "astar_process", 0.05);
      usleep(1000);
    }
    super_planner::rog_map::Vec3f cur_pos;
    globalIndexToPos(current->id_g, cur_pos);
    super_planner::rog_map::GridType cur_inf_type = map_ptr_->getInfGridType(cur_pos);
    if (md_.unknown_as_occ && cur_inf_type != OCCUPIED &&
        cur_inf_type != UNKNOWN) {
      retrievePath(current, node_path);
      ConvertNodePathToPointPath(node_path, out_path);
      //                double time_2 = ros_ptr_->getSimTime();
      //                    printf("\033[34m Escape: A star iter:%d, time:%.3f
      //                    ms\033[0m\n", num_iter, (time_2 - time_1).toSec() *
      //                    1000);
      return REACH_HORIZON;
    }
    ...
    if (!cfg_.visual_process && (time_2 - time_1) > 0.2) {
      SLOG_WARN("Failed in A star path searching !!! 0.2 seconds time limit "
                "exceeded.");
      return TIME_OUT;
    }
  }
  ...
  SLOG_WARN(" -- [A*] Escape path searcher, cannot find path, return.");
  return NO_PATH;
}

bool Astar::neighborHaveOne(const super_planner::rog_map::GridType &type,
                            const super_planner::rog_map::Vec3i &src_id) {
  for (const auto &nei : neighbor_list) {
    super_planner::rog_map::Vec3i nei_id = src_id + nei;
    if (!insideLocalMap(nei_id)) {
      continue;
    }
    super_planner::rog_map::Vec3f nei_pos;
    globalIndexToPos(nei_id, nei_pos);
    super_planner::rog_map::GridType nei_type;
    if (md_.use_inf_map) {
    ...
    /* Cells outside the sliding map must fail the dilated-occupancy test:
     * without this, the KNOWN_FREE neighborhood check leaks across the
     * map boundary and A* paths into unmapped space the corridor
     * generator (strict insideLocalMap) cannot cover. */
    if (type == OCCUPIED && nei_type == OUT_OF_MAP) {
      return true;
    }
  }
  return false;
...
} // namespace super_planner // namespace path_search
