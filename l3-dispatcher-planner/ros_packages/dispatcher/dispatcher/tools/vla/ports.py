"""VLA 技能宿主端口：通用 SkillHost 端口 + 几何原语源访问。

l3-skill-contract.spec.md §7/§10：VLA 技能使用通用 SkillHost 端口
（最新帧、快速通道图像、动作武装、航点下发、模式发布、相位上报、
任务结果暂存、失败终止、代次守卫、队列机械操作），不新增飞行专属
端口；航点塑形（高度限幅、mode 脉冲）由技能内联完成。本协议在通用
端口之上只增加一项 VLA 组合注入所需的访问：几何原语源
（VlaGeometrySource，perception/base_policy 实例经宿主组合注入到达）。
...
class VlaHostConfig:
    """VLA 宿主只读配置与出站回调；默认值与旧库 config.py 核对一致。

    publish_planner_mode 为模式脉冲的出站回调（旧链 planner_mode_topic 的
    Int32 发布）。EGO 直连栈当前没有模式触发通道，装配层（P3）必须显式
    接通真实出口或提供受控空实现；未接通时 publish_mode_burst 直接失败，
    不静默降级。
    """

    min_height: float = 0.0  # 旧 config 最低允许飞行高度（clamp 下界，米）
    max_height: float = 1.8  # 旧 config 最高允许飞行高度（clamp 上界，米）
    planner_ego_mode_value: int = 1  # 旧 config ego 模式枚举值（mode_burst 入参）
    planner_mode_repeat: int = 3  # 旧 config 模式脉冲重复次数
    planner_mode_interval: float = 0.03  # 旧 config 模式脉冲间隔（秒）
...
class VlaSkillHost(Protocol):
    ...
    def latest_frame(self) -> Any: ...                       # 最新感知帧（Frame | None）
    def get_fast_rgb(self) -> tuple: ...                     # 快通道 RGB 二元组 (image, stamp)
    def geometry_source(self) -> VlaGeometrySource: ...      # 几何原语源；GeometryService 的注入端口
    ...
    def fail_sequence(self, reason: str) -> None: ...        # 任务序列失败唯一写入口（fail-closed 三态经 reason 传入）
