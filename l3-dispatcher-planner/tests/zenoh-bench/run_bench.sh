#!/bin/bash
# Entity: devel — launch the l3 zenoh-bench container and run the routing matrix.
# Usage: bash tests/zenoh-bench/run_bench.sh   (from the l3 project root)
set -euo pipefail

IMG="l3-dispatcher-planner-real/amd64-ubuntu20.04-cuda0.0.0:latest"
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
ROS_MASTER_PORT="${ROS_MASTER_PORT:-29311}"
...
exec docker run --rm --network host \
  ...
  -v "$ROOT/.artifacts/bench-workdir:/tmp/zenoh-bench:rw" \
  --entrypoint bash "$IMG" -c '
    source /opt/ros/noetic/setup.bash
    source /workspace/devel/setup.bash
    export ROS_PACKAGE_PATH=/workspace/src:$ROS_PACKAGE_PATH
    python3 -u /workspace/src/tests/zenoh-bench/verify_routing.py
  '
